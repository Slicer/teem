...
teem/gage.h, generated by build_teem.py to declare the gage
...
/* defaultsGage.c */
extern const char *const gageBiffKey;
extern int gageDefVerbose;
extern double gageDefGradMagCurvMin;
extern int gageDefRenormalize;
extern int gageDefCheckIntegrals;
extern int gageDefK3Pack;
extern int gageDefCurvNormalSide;
extern double gageDefKernelIntegralNearZero;
extern int gageDefDefaultCenter;
extern int gageDefStackUse;
extern int gageDefStackNormalizeRecon;
extern int gageDefStackNormalizeDeriv;
extern double gageDefStackNormalizeDerivBias;
extern int gageDefOrientationFromSpacing;
extern int gageDefGenerateErrStr;
extern int gageDefTwoDimZeroZ;
/* miscGage.c */
extern const int gagePresent;
extern double gageZeroNormal[3];
extern const airEnum *const gageErr;
extern const airEnum *const gageKernel;
extern const airEnum *const gageItemPackPart;
...
extern void gageItemSpecInit(gageItemSpec *isp);
extern gageItemSpec *gageItemSpecNix(gageItemSpec *isp);
/* kind.c */
extern int gageKindCheck(const gageKind *kind);
extern unsigned int gageKindTotalAnswerLength(const gageKind *kind);
extern unsigned int gageKindAnswerLength(const gageKind *kind, int item);
extern int gageKindAnswerOffset(const gageKind *kind, int item);
extern int gageKindVolumeCheck(const gageKind *kind, const Nrrd *nrrd);
/* print.c */
extern void gageQueryPrint(FILE *file, const gageKind *kind, gageQuery query);
/* sclfilter.c */
...
extern gageScl3PFilter_t gageScl3PFilter2;
extern gageScl3PFilter_t gageScl3PFilter4;
extern gageScl3PFilter_t gageScl3PFilter6;
extern gageScl3PFilter_t gageScl3PFilter8;
extern void gageScl3PFilterN(gageShape *shape, int fd, double *iv3, double *iv2,
                                  double *iv1, double *fw00, double *fw11, double *fw22,
                                  double *val, double *gvec, double *hess,
                                  const int *needD);
/* scl.c */
extern const airEnum *const gageScl;
...
extern gageKind *const gageKindScl;
extern const gageItemPack *const gageItemPackSclValue;
...
extern const airEnum *const gageVec;
extern gageKind *const gageKindVec;
/* twovecGage.c */
extern const airEnum *const gage2Vec;
extern gageKind *const gageKind2Vec;
/* shape.c */
extern void gageShapeReset(gageShape *shp);
extern gageShape *gageShapeNew(void);
extern gageShape *gageShapeCopy(const gageShape *shp);
extern gageShape *gageShapeNix(gageShape *shape);
extern int gageShapeSet(gageShape *shp, const Nrrd *nin, int baseDim);
extern void gageShapeWtoI(const gageShape *shape, double index[3],
                               const double world[3]);
extern void gageShapeItoW(const gageShape *shape, double world[3],
                               const double index[3]);
extern int gageShapeEqual(const gageShape *shp1, const char *name1,
                               const gageShape *shp2, const char *name2);
extern void gageShapeBoundingBox(double min[3], double max[3],
...
/* pvl.c */
extern int gageVolumeCheck(const gageContext *ctx, const Nrrd *nin,
                                const gageKind *kind);
extern gagePerVolume *gagePerVolumeNew(gageContext *ctx,
                                            const Nrrd *nin,
                                            const gageKind *kind);
extern gagePerVolume *gagePerVolumeNix(gagePerVolume *pvl);
extern const double *gageAnswerPointer(const gageContext *ctx,
                                            const gagePerVolume *pvl,
                                            int item);
extern unsigned int gageAnswerLength(const gageContext *ctx,
                                          const gagePerVolume *pvl,
                                          int item);
...
extern int gageQueryAdd(gageContext *ctx, gagePerVolume *pvl, gageQuery query);
extern int gageQueryItemOn(gageContext *ctx, gagePerVolume *pvl, int item);
/* optimsig.c */
extern int gageOptimSigSet(double *scale, unsigned int num, unsigned int sigmaMax);
extern gageOptimSigContext *gageOptimSigContextNew(unsigned int dim,
                                                        unsigned int sampleNumMax,
                                                        unsigned int trueImgNum,
                                                        double sigmaMin, double sigmaMax,
                                                        double cutoff);
extern gageOptimSigContext *gageOptimSigContextNix(gageOptimSigContext *oscx);
extern int gageOptimSigCalculate(gageOptimSigContext *oscx,
                                      /* output */ double *sigma, unsigned int sigmaNum,
                                      const NrrdKernelSpec *kssSpec, int imgMeasr,
                                      int allMeasr, unsigned int maxIter,
                                      double convEps);
extern int gageOptimSigErrorPlot(gageOptimSigContext *oscx,
                                      Nrrd *nout,
                                      const double *sigma,
                                      unsigned int sigmaNum,
                                      const NrrdKernelSpec *kssSpec,
                                      int imgMeasr);
extern int gageOptimSigErrorPlotSliding(gageOptimSigContext *oscx,
                                             Nrrd *nout,
                                             double windowRho,
...
/* moved the tau conversion functions to air, so that they can be
   called from unu */
extern double gageStackWtoI(gageContext *ctx, double swrl, int *outside);
extern double gageStackItoW(gageContext *ctx, double si, int *outside);
extern int gageStackPerVolumeNew(gageContext *ctx,
                                      gagePerVolume **pvlStack,
                                      const Nrrd *const *nblur,
                                      unsigned int blnum,
                                      const gageKind *kind);
extern int gageStackPerVolumeAttach(gageContext *ctx,
                                         gagePerVolume *pvlBase,
                                         gagePerVolume **pvlStack,
                                         const double *stackPos,
                                         unsigned int blnum);
extern int gageStackProbe(gageContext *ctx, double xi, double yi, double zi,
                               double si);
extern int gageStackProbeSpace(gageContext *ctx, double x, double y, double z,
                                    double s, int indexSpace, int clamp);
/* stackBlur.c */
extern const airEnum *const gageSigmaSampling;
extern gageStackBlurParm *gageStackBlurParmNew(void);
extern int gageStackBlurParmCopy(gageStackBlurParm *sbpDst,
                                      const gageStackBlurParm *sbpSrc);
extern void gageStackBlurParmInit(gageStackBlurParm *parm);
extern gageStackBlurParm *gageStackBlurParmNix(gageStackBlurParm *sbp);
extern int gageStackBlurParmCompare(const gageStackBlurParm *sbpA,
                                         const char *nameA,
...
extern int gageStackBlurParmSigmaSet(gageStackBlurParm *sbp,
                                          unsigned int num,
                                          double sigmaMin,
                                          double sigmaMax,
                                          int sigmaSampling);
extern int gageStackBlurParmKernelSet(gageStackBlurParm *sbp,
                                           const NrrdKernelSpec *kspec);
extern int gageStackBlurParmRenormalizeSet(gageStackBlurParm *sbp, int renormalize);
extern int gageStackBlurParmDgGoodSigmaMaxSet(gageStackBlurParm *sbp,
                                                   double dgGoodSigmaMax);
extern int gageStackBlurParmBoundarySet(gageStackBlurParm *sbp, int boundary,
                                             double padValue);
extern int gageStackBlurParmBoundarySpecSet(gageStackBlurParm *sbp,
                                                 const NrrdBoundarySpec *bsp);
extern int gageStackBlurParmNeedSpatialBlurSet(gageStackBlurParm *sbp, int sblur);
extern int gageStackBlurParmVerboseSet(gageStackBlurParm *sbp, int verbose);
extern int gageStackBlurParmOneDimSet(gageStackBlurParm *sbp, int oneDim);
extern int gageStackBlurParmCheck(const gageStackBlurParm *sbp);
extern int gageStackBlurParmParse(gageStackBlurParm *sbp,
                                       int extraFlags[256],
                                       char **extraParmsP,
                                       const char *str);
...
extern int gageStackBlurParmSprint(char str[AIR_STRLEN_LARGE],
                                        ...
                                        int extraFlag[256],
                                        char *extraParm);
extern int gageStackBlur(Nrrd *const nblur[], gageStackBlurParm *sbp,
                              const Nrrd *nin, const gageKind *kind);
extern int gageStackBlurCheck(const Nrrd *const nblur[], gageStackBlurParm *sbp,
                                   const Nrrd *nin, const gageKind *kind);
extern int gageStackBlurGet(Nrrd *const nblur[], int *recomputedP,
                                 gageStackBlurParm *sbp, const char *format,
                                 const Nrrd *nin, const gageKind *kind);
extern int gageStackBlurManage(Nrrd ***nblurP, int *recomputedP,
                                    gageStackBlurParm *sbp, const char *format,
                                    int saveIfComputed, NrrdEncoding *enc,
                                    const Nrrd *nin, const gageKind *kind);
/* ctx.c */
extern gageContext *gageContextNew(void);
extern gageContext *gageContextCopy(gageContext *ctx);
extern gageContext *gageContextNix(gageContext *ctx);
extern void gageParmSet(gageContext *ctx, int which, double val);
extern int gagePerVolumeIsAttached(const gageContext *ctx,
                                        const gagePerVolume *pvl);
...
extern int gageKernelSet(gageContext *ctx, int which, const NrrdKernel *k,
                              const double *kparm);
extern void gageKernelReset(gageContext *ctx);
extern int gageProbe(gageContext *ctx, double xi, double yi, double zi);
extern int gageProbeSpace(gageContext *ctx, double x, double y, double z,
                               int indexSpace, int clamp);
/* update.c */
extern int gageUpdate(gageContext *ctx);
/* st.c */
extern int gageStructureTensor(Nrrd *nout, const Nrrd *nin, int dScale, int iScale,
                                    int dsmp);
/* deconvolve.c */
extern int gageDeconvolve(Nrrd *nout, double *lastDiffP, const Nrrd *nin,
...
extern int gageDeconvolveSeparableKnown(const NrrdKernelSpec *ksp);
extern int gageDeconvolveSeparable(Nrrd *nout, const Nrrd *nin,
                                        const gageKind *kind, const NrrdKernelSpec *ksp,
                                        int typeOut);
