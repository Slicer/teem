...
teem/push.h, generated by build_teem.py to declare the push
...
/* defaultsPush.c */
extern const int pushPresent;
extern const char *const pushBiffKey;
/* methodsPush.c */
extern pushPoint *pushPointNew(pushContext *pctx);
extern pushPoint *pushPointNix(pushPoint *pnt);
extern pushContext *pushContextNew(void);
extern pushContext *pushContextNix(pushContext *pctx);
/* forces.c (legacy name for info about (derivatives of) energy functions) */
extern const airEnum *const pushEnergyType;
extern const pushEnergy *const pushEnergyUnknown;
extern const pushEnergy *const pushEnergySpring;
extern const pushEnergy *const pushEnergyGauss;
extern const pushEnergy *const pushEnergyCoulomb;
extern const pushEnergy *const pushEnergyCotan;
extern const pushEnergy *const pushEnergyZero;
extern const pushEnergy *const pushEnergyAll[PUSH_ENERGY_TYPE_MAX + 1];
extern pushEnergySpec *pushEnergySpecNew(void);
extern void pushEnergySpecSet(pushEnergySpec *ensp,
                                   const pushEnergy *energy,
                                   const double parm[PUSH_ENERGY_PARM_NUM]);
extern pushEnergySpec *pushEnergySpecNix(pushEnergySpec *ensp);
extern int pushEnergySpecParse(pushEnergySpec *ensp, const char *str);
extern const hestCB *const pushHestEnergySpec;
/* corePush.c */
extern int pushStart(pushContext *pctx);
extern int pushIterate(pushContext *pctx);
extern int pushRun(pushContext *pctx);
extern int pushFinish(pushContext *pctx);
/* binning.c */
extern void pushBinInit(pushBin *bin, unsigned int incr);
extern void pushBinDone(pushBin *bin);
extern int pushBinPointAdd(pushContext *pctx, pushPoint *point);
extern void pushBinAllNeighborSet(pushContext *pctx);
extern int pushRebin(pushContext *pctx);
/* action.c */
extern int pushBinProcess(pushTask *task, unsigned int myBinIdx);
extern int pushOutputGet(Nrrd *nPos, Nrrd *nTen, Nrrd *nEnr, pushContext *pctx);
