...
alanInit(alanContext *actx, const Nrrd *nlevInit, const Nrrd *nparmInit) {
  ...
  if (nlevInit) {
    ...
    if (!(alan_nt == nlevInit->type && nlevInit->dim == 1 + actx->dim
  ...
  if (nparmInit) {
    ...
    if (!(alan_nt == nparmInit->type && nparmInit->dim == 1 + actx->dim
          && 3 == nparmInit->axis[0].size && actx->size[0] == nparmInit->axis[1].size
...
_alanTuringWorker(void *_task) {
  ...
  for (iter = 0; (alanStopNot == task->actx->stop
                  && (0 == task->actx->maxIteration || iter < task->actx->maxIteration));
       iter++) {
...
alanRun(alanContext *actx) {
  ...
  if (!(actx->_nlev[0] && actx->_nlev[0])) {
    biffAddf(ALAN,
             "%s: _nlev[0,1] not allocated: "
             "call alanUpdate + alanInit",
             me);
    return 1;
  }
  ...
  for (tid = 0; tid < actx->numThreads; tid++) {
    ...
    airThreadStart(task[tid].thread, _alanTuringWorker, (void *)&(task[tid]));
  }
  for (tid = 0; tid < actx->numThreads; tid++) {
