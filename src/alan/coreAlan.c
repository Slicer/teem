...
_alanStart(alanContext *actx) {
  ...
  actx->nlev[1] = nrrdNew();
  if (2 == actx->dim) {
    ret = (nrrdMaybeAlloc(actx->nlev[0], nrrdTypeDouble, 3,
			  3, actx->size[0], actx->size[1])
	   || nrrdMaybeAlloc(actx->nlev[1], nrrdTypeDouble, 3,
			     3, actx->size[1], actx->size[1]));
  } else {
    ret = (nrrdMaybeAlloc(actx->nlev[0], nrrdTypeDouble, 4,
			  3, actx->size[0], actx->size[1], actx->size[2])
	   || nrrdMaybeAlloc(actx->nlev[1], nrrdTypeDouble, 4,
  ...
  lev0 = (double*)(actx->nlev[0]->data);
  lev1 = (double*)(actx->nlev[1]->data);
  for (I=0; I<N; I++) {
    lev0[0 + 3*I] = actx->initA;
    lev0[1 + 3*I] = actx->initB;
    lev0[2 + 3*I] = AIR_AFFINE(0, airRand(), 1,
...
_alanTuring2DIter(alanContext *actx) {
  double *lev0, *lev1, *v[9], lapA, lapB, deltaA, deltaB;
  int idx, px, mx, py, my, sx, sy, x, y;

  sx = actx->size[0];
  sy = actx->size[1];
  lev0 = (double*)(actx->nlev[actx->iter % 2]->data);
  lev1 = (double*)(actx->nlev[(actx->iter+1) % 2]->data);
  ...
  for (y=0; y<sy; y++) {
    ...
    for (x=0; x<sx; x++) {
      ...
      deltaB = (actx->K*(v[4][0]*v[4][1] - v[4][1] - 12 + v[4][2]) 
		+ actx->diffB*lapB);
      
      lev1[0 + 3*idx] = actx->speed*deltaA + lev0[0 + 3*idx];
...
alanRun(alanContext *actx) {
  ...
  for (actx->iter=0; actx->iter<actx->maxIteration; actx->iter++) {
    ...
    if (actx->saveInterval && !(actx->iter % actx->saveInterval)) {
      nrrdSave("lev.nrrd", actx->nlev[actx->iter % 2], NULL);
    }
    iter(actx);
  }
