...
static const char *probeInfo = ("Uses gageProbe() to query scalar or vector volumes "
...
main(int argc, const char *argv[]) {
  ...
  if (numSS) {
    ...
    if (gageStackBlurParmScaleSet(sbp, numSS, rangeSS[0], rangeSS[1], SSuniform,
                                  AIR_FALSE)
  ...
  if (numSS) {
    ...
    gageParmSet(ctx, gageParmStackNormalizeDeriv, SSnormd ? AIR_TRUE : AIR_FALSE);
    if (!E)
  ...
  if (lpld) {
    ...
    for (vidx = 0; vidx < lpld->xyzwNum; vidx++) {
      ...
      if (gageProbeSpace(ctx, xyzw[0], xyzw[1], xyzw[2], !worldSpace, AIR_TRUE)) {
    ...
    for (vidx = 0; vidx < lineStepNum; vidx++) {
      ...
      if (gageProbeSpace(ctx, lpos[0], lpos[1], lpos[2], !worldSpace, AIR_FALSE)) {
        fprintf(stderr, "%s: trouble:\n%s\n(%d)\n", me, ctx->errStr, ctx->errNum);
        airMopError(mop);
        return 1;
      }
    ...
    E = (numSS ? gageStackProbeSpace(ctx, pos[0], pos[1], pos[2], posSS, !worldSpace,
                                     AIR_FALSE)
               : gageProbeSpace(ctx, pos[0], pos[1], pos[2], !worldSpace, AIR_FALSE));
    if (E) {
      fprintf(stderr, "%s: trouble:\n%s\n(%d)\n", me, ctx->errStr, ctx->errNum);
      airMopError(mop);
      return 1;
    }
    printf("%s: %s(%g,%g,%g) = ", me, airEnumStr(kind->enm, what), pos[0], pos[1],
           pos[2]);
...
#define PROBE(x, y, z)                                                                  \
  ((numSS ? gageStackProbeSpace(ctx, x, y, z, posSS, !worldSpace, AIR_FALSE)            \
          : gageProbeSpace(ctx, x, y, z, !worldSpace, AIR_FALSE)),                      \
   answer[0])
      for (xo = 0; xo <= 2; xo++) {
        for (yo = 0; yo <= 2; yo++) {
          for (zo = 0; zo <= 2; zo++) {
      ...
      printf("%s: approx hessian(%s) at (%g,%g,%g) = \n"
             ...
             "   %f %f\n"
             "      %f\n",
             me, airEnumStr(kind->enm, what), pos[0], pos[1], pos[2],
