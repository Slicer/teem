...
qbertSizeUp(Nrrd *nout, Nrrd *nin, unsigned int *sz, NrrdKernelSpec *uk) {
  ...
  if (uk) {
    for (i = 0; i <= 2; i++) {
      ...
      fprintf(stderr, "%s: sz[%d] = %u -> need = %d --> ", me, i,
              AIR_UINT(nin->axis[i].size), need);
      need = AIR_MAX(0, need);
    ...
    if (anyneed) {
      ...
      rsi->clamp = AIR_TRUE;
      fprintf(stderr, "%s: resampling ... ", me);
      fflush(stderr);
    ...
    for (i = 0; i <= 2; i++) {
      ...
      fprintf(stderr, "pad indices: [%d..%d]\n", AIR_INT(padMin[i]), AIR_INT(padMax[i]));
    }
    if (anyneed) {
      fprintf(stderr, "%s: padding ... ", me);
      fflush(stderr);
  ...
  for (i = 0; i <= 2; i++) {
    if (nin->axis[i].size > sz[i]) {
      ...
      fprintf(stderr, "%s: downsampling axis %d from %s to %s samples\n", me, i,
              airSprintSize_t(stmp1, nin->axis[i].size),
...
qbertMakeVghHists(Nrrd *nvhist, Nrrd *nghist, Nrrd *nhhist, unsigned int *sz, int bins,
  ...
  nvhist->axis[0].min = minv;
  nvhist->axis[0].max = maxv;
  nghist->axis[0].min = ming;
  nghist->axis[0].max = maxg;
  vhist = (int *)nvhist->data;
  ghist = (int *)nghist->data;
  memset(vhist, 0, bins * sizeof(int));
  memset(ghist, 0, bins * sizeof(int));
  if (doH) {
    nhhist->axis[0].min = minh;
    nhhist->axis[0].max = maxh;
...
qbertMakeVgh(Nrrd *nvgh, Nrrd *nvhist, Nrrd *nghist, Nrrd *nhhist, unsigned int *sz,
  ...
  nval = AIR_UINT(nvghF->axis[0].size);
  doH = !!(nval == 3);
  minv = nvhist->axis[0].min;
  maxv = nvhist->axis[0].max;
  ming = nghist->axis[0].min;
  maxg = nghist->axis[0].max;
  vhist = (int *)nvhist->data;
  ghist = (int *)nghist->data;
  if (doH) {
    minh = nhhist->axis[0].min;
    maxh = nhhist->axis[0].max;
    hhist = (int *)nhhist->data;
  }
  ...
  fprintf(stderr, "%s: new values (ignored %5d): [%g .. %g]\n", me,
          (int)(perc[0] * sz[0] * sz[1] * sz[2] / 100), minv, maxv);
  fprintf(stderr, "%s: new  grads (ignored %5d): [%g .. %g]\n", me,
          (int)(perc[1] * sz[0] * sz[1] * sz[2] / 100), ming, maxg);
  if (doH) {
    fprintf(stderr, "%s: new 2ndDDs (ignored %5d): [%g .. %g]\n", me,
            (int)(perc[2] * sz[0] * sz[1] * sz[2] / 100), minh, maxh);
  ...
  sprintf(cmt, "minv: %g", minv);
  nrrdCommentAdd(nvgh, cmt);
  sprintf(cmt, "maxv: %g", maxv);
  nrrdCommentAdd(nvgh, cmt);
  sprintf(cmt, "ming: %g", ming);
  nrrdCommentAdd(nvgh, cmt);
  sprintf(cmt, "maxg: %g", maxg);
  nrrdCommentAdd(nvgh, cmt);
  if (doH) {
    sprintf(cmt, "minh: %g", minh);
    nrrdCommentAdd(nvgh, cmt);
    sprintf(cmt, "maxh: %g", maxh);
    nrrdCommentAdd(nvgh, cmt);
  }
  nrrdAxisInfoSet_va(nvgh, nrrdAxisInfoCenter, nrrdCenterUnknown, nrrdCenterNode,
...
qbertScat(Nrrd *nvgh, int pos, unsigned int size, const char *name) {
  ...
  if (!E)
    E |= nrrdHistoJoint(nscA, (const Nrrd *const *)nin, NULL, 2, NULL, bins,
                        nrrdTypeFloat, clamp);
...
main(int argc, const char *argv[]) {
  ...
  for (i = 0; i <= 2; i++) {
    if (!AIR_EXISTS(nin->axis[i].min)) amin[1 + i] = AIR_NAN;
    if (!AIR_EXISTS(nin->axis[i].max)) amax[1 + i] = AIR_NAN;
  }
  ...
  if (useFloat) {
    ...
    if (qbertMakeVghHists(nvhist, nghist, nhhist, sz, QBERT_HIST_BINS, nvghF, nin)) {
    ...
    nrrdAxisInfoSet_va(nvgh, nrrdAxisInfoLabel, !notdoH ? "vgh" : "vg", "x", "y", "z");
