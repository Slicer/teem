...
BNDS_TMPL(Sphere, lo[0] = obj->pos[0] - obj->rad; lo[1] = obj->pos[1] - obj->rad;
          lo[2] = obj->pos[2] - obj->rad; hi[0] = obj->pos[0] + obj->rad;
          hi[1] = obj->pos[1] + obj->rad; hi[2] = obj->pos[2] + obj->rad;)

BNDS_TMPL(Cylinder, AIR_UNUSED(obj); ELL_3V_SET(lo, -1, -1, -1);
          ELL_3V_SET(hi, 1, 1, 1);)

BNDS_TMPL(Superquad, AIR_UNUSED(obj); ELL_3V_SET(lo, -1, -1, -1);
          ELL_3V_SET(hi, 1, 1, 1);)

BNDS_TMPL(Cube, AIR_UNUSED(obj); ELL_3V_SET(lo, -1, -1, -1); ELL_3V_SET(hi, 1, 1, 1);)

BNDS_TMPL(Triangle, ELL_3V_COPY(lo, obj->vert[0]); ELL_3V_MIN(lo, lo, obj->vert[1]);
          ELL_3V_MIN(lo, lo, obj->vert[2]); ELL_3V_COPY(hi, obj->vert[0]);
          ELL_3V_MAX(hi, hi, obj->vert[1]); ELL_3V_MAX(hi, hi, obj->vert[2]);)

BNDS_TMPL(Rectangle, echoPos_t v[3][3];

          ELL_3V_COPY(lo, obj->origin); ELL_3V_ADD2(v[0], lo, obj->edge0);
          ELL_3V_ADD2(v[1], lo, obj->edge1); ELL_3V_ADD2(v[2], v[0], obj->edge1);
          ELL_3V_MIN(lo, lo, v[0]); ELL_3V_MIN(lo, lo, v[1]); ELL_3V_MIN(lo, lo, v[2]);
          ELL_3V_COPY(hi, obj->origin); ELL_3V_MAX(hi, hi, v[0]);
          ELL_3V_MAX(hi, hi, v[1]); ELL_3V_MAX(hi, hi, v[2]);)

BNDS_TMPL(TriMesh, ELL_3V_COPY(lo, obj->min); ELL_3V_COPY(hi, obj->max);)

BNDS_TMPL(Isosurface, AIR_UNUSED(obj);
          fprintf(stderr, "_echoIsosurface_bounds: unimplemented!\n");)

BNDS_TMPL(AABBox, ELL_3V_COPY(lo, obj->min); ELL_3V_COPY(hi, obj->max);)

BNDS_TMPL(
  List, unsigned int i; echoPos_t l[3]; echoPos_t h[3]; echoObject * o;
...
BNDS_TMPL(Split, AIR_UNUSED(obj);
...
static const _echoBoundsGet_t _echoBoundsGet[ECHO_TYPE_NUM] = {
  ...
  (_echoBoundsGet_t)_echoTriMesh_bounds,   (_echoBoundsGet_t)_echoIsosurface_bounds,
  (_echoBoundsGet_t)_echoAABBox_bounds,    (_echoBoundsGet_t)_echoSplit_bounds,
  (_echoBoundsGet_t)_echoList_bounds,      (_echoBoundsGet_t)_echoInstance_bounds,
};
