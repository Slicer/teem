...
echoThreadStateInit(int threadIdx, echoThreadState *tstate, echoRTParm *parm,
  ...
  nrrdAxisInfoSet_va(tstate->njitt, nrrdAxisInfoLabel, "x,y", "jittable", "sample");
  ...
  if (!(tstate->chanBuff = (echoCol_t *)calloc(ECHO_IMG_CHANNELS * parm->numSamples,
                                               sizeof(echoCol_t)))) {
    biffAddf(ECHO, "%s: couldn't allocate img channel sample buffer", me);
    return 1;
  }

  airSrandMT_r(tstate->rst, AIR_UINT((parm->seedRand ? airTime() : threadIdx)));
...
echoJitterCompute(echoRTParm *parm, echoThreadState *tstate) {
  ...
  for (j = 0; j < ECHO_JITTABLE_NUM; j++) {
    airShuffle_r(tstate->rst, tstate->permBuff, parm->numSamples, parm->permuteJitter);
    for (s = 0; s < N; s++) {
...
echoRTRenderCheck(Nrrd *nraw, limnCamera *cam, echoScene *scene, echoRTParm *parm,
  ...
  if (!(parm->imgResU > 0 && parm->imgResV)) {
    biffAddf(ECHO, "%s: image dimensions (%dx%d) invalid", me, parm->imgResU,
             parm->imgResV);
    return 1;
  }
  ...
  case echoJitterJitter:
    ...
    if (tmp * tmp != parm->numSamples) {
      biffAddf(ECHO, "%s: need a square # samples for %s jitter method (not %d)", me,
               airEnumStr(echoJitter, parm->jitterType), parm->numSamples);
      return 1;
    }
    break;
  }
...
void
echoChannelAverage(echoCol_t *img, echoRTParm *parm, echoThreadState *tstate) {
  int s;
  echoCol_t R, G, B, A, T;
...
echoRayColor(echoCol_t chan[4], echoRay *ray, echoScene *scene, echoRTParm *parm,
  ...
  if (tstate->depth > parm->maxRecDepth) {
    ...
    ELL_4V_SET(chan, parm->maxRecCol[0], parm->maxRecCol[1], parm->maxRecCol[2], 1.0);
    goto done;
  }
  ...
  if (!echoRayIntx(&intx, ray, scene, parm, tstate)) {
    ...
    ELL_4V_SET_TT(chan, echoCol_t, scene->bkgr[0], scene->bkgr[1], scene->bkgr[2],
                  (parm->renderBoxes ? 1.0 - pow(1.0 - parm->boxOpac, intx.boxhits)
                                     : 0.0));
    goto done;
  }
  ...
  if (tstate->verbose) {
    fprintf(stderr,
            "%s%s: hit a %d (%p) at (%g,%g,%g)\n"
            "%s    = %g along (%g,%g,%g)\n",
            _echoDot(tstate->depth), me, intx.obj->type, AIR_VOIDP(intx.obj),
            intx.pos[0], intx.pos[1], intx.pos[2], _echoDot(tstate->depth), intx.t,
...
_echoRTRenderThreadBody(void *_arg) {
  ...
  echoPos_t tmp0, tmp1, pixUsz, pixVsz, /* U and V dimensions of a pixel */
  ...
  while (1) {
    ...
    imgV = NRRD_POS(nrrdCenterCell, cam->vRange[0], cam->vRange[1], parm->imgResV,
                    imgVi);
    for (imgUi = 0; imgUi < parm->imgResU; imgUi++) {
      imgU = NRRD_POS(nrrdCenterCell, cam->uRange[0], cam->uRange[1], parm->imgResU,
                      imgUi);
...
echoRTRender(Nrrd *nraw, limnCamera *cam, echoScene *scene, echoRTParm *parm,
  ...
  nrrdAxisInfoSet_va(nraw, nrrdAxisInfoLabel, "r,g,b,a,t", "x", "y");
  nrrdAxisInfoSet_va(nraw, nrrdAxisInfoMin, AIR_NAN, cam->uRange[0], cam->vRange[0]);
  nrrdAxisInfoSet_va(nraw, nrrdAxisInfoMax, AIR_NAN, cam->uRange[1], cam->vRange[1]);
  gstate->time = airTime();

  if (parm->numThreads > 1) {
    gstate->workMutex = airThreadMutexNew();
    airMopAdd(mop, gstate->workMutex, (airMopper)airThreadMutexNix, airMopAlways);
  } else {
