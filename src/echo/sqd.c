...
_echo_SuperquadX_lvg(echoPos_t grad[3], echoPos_t x, echoPos_t y, echoPos_t z,
  ...
  if (!( AIR_EXISTS(grad[0]) && AIR_EXISTS(grad[1]) && AIR_EXISTS(grad[2]) )) {
    ...
    fprintf(stderr, "R: pow(%g + %g = %g, 1-(%g/%g = %g) = %g) = %g*%g = %g\n",
            ...
            pow(yya + zza, 1-(A/B))*xxb);
    fprintf(stderr, "grad[0]: 2/(%g * %g * (1 + pow(%g + %g = %g, %g/%g = %g)/%g = %g)) =
  %g\n", B, x, yya, zza, yya+zza, A, B, A/B, xxb, pow(yya + zza, A/B)/xxb, grad[0]);
...
_echoRayIntx_Superquad(RAYINTX_ARGS(Superquad)) {
  ...
  echoPos_t TT = 0, Tmin, Tmax, t0, t1, t2, t3, v1, v2, diff, tol, saveTmin, Vmin, Vmax,
            ...
            (*vg)(echoPos_t[3], echoPos_t, echoPos_t, echoPos_t, echoPos_t, echoPos_t),
            (*lvg)(echoPos_t[3], echoPos_t, echoPos_t, echoPos_t, echoPos_t, echoPos_t),
            from[3], grad[3], pos[3]; /* these two used only by macros */
  int iter;

  if (!_echoRayIntx_CubeSolid(&Tmin, &Tmax, -1 - 2 * ECHO_EPSILON, 1 + 2 * ECHO_EPSILON,
...
#define SHIFT2(a, b, c)                                                                 \
  ...
  */
  ...
  if (dVmin * dVmax < 0 && Vmin * Vmax >= 0) {
    ...
    if (tstate->verbose) {
      fprintf(stderr,
              ...
              "     t2 = % 31.15f  -> v2 = % 31.15f\n"
              "     t3 = % 31.15f\n",
              _echoDot(tstate->depth), me, t0, t1, v1, t2, v2, t3);
    }
    ...
    while ((t3 - t0 > tol * (t1 + t2)) /* still haven't converged */
      ...
      if (tstate->verbose) {
        fprintf(stderr,
                ...
                "     t2 = % 31.15f  -> v2 = % 31.15f\n"
                "     t3 = % 31.15f\n",
                _echoDot(tstate->depth), me, diff > 0 ? "v1 < v2" : "v1 > v2", t0, t1,
                v1, t2, v2, t3);
      }
    }
    if (v1 > 0 && v2 > 0) {
  ...
  while (iter < parm->sqNRI && AIR_ABS(VV) > parm->sqTol && AIR_EXISTS(VV)
         && AIR_EXISTS(dV)) {
    if (tstate->verbose) {
