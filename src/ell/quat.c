...
#define _ELL_AA_TO_Q(type)                                                              \
  ...
  ELL_4V_SET(q, AIR_CAST(type, cos(angle / 2)), AIR_CAST(type, sa *axis[0]),            \
...
ell_q_avg4_d(double m[4], unsigned int *iterP, const double _q1[4], const double _q2[4],
  ...
  double N, elen, a[4], b[4], c[4], d[4], tmp[4], la[4], lb[4], lc[4], ld[4], u[4],
    wght[4];
  unsigned int iter;
  ...
  if (elen > eps) {
    biffAddf(ELL, "%s: still have error %g after max %d iters", me, elen, maxIter);
    return 1;
  }
...
ell_q_avgN_d(double mm[4], unsigned int *iterP, const double *qq, double *qlog,
  ...
  if (elen > eps) {
    biffAddf(ELL, "%s: still have error %g (> eps %g) after max %d iters", me, elen, eps,
             maxIter);
    return 1;
  }
