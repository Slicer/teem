...
**
*/
#define GAGE_DV_SET(v, d0, d1, d2) ((v)[0] = (d0), (v)[1] = (d1), (v)[2] = (d2))

#define GAGE_DV_EQUAL(a, b) ((a)[0] == (b)[0] && (a)[1] == (b)[1] && (a)[2] == (b)[2])

#define GAGE_DV_COPY(a, b) ((a)[0] = (b)[0], (a)[1] = (b)[1], (a)[2] = (b)[2])
...
typedef struct {
  ...
  unsigned int answerLength; /* how many double's are needed to store the answer,
...
/* sclfilter.c */
typedef void(gageScl3PFilter_t)(gageShape *shape, double *iv3, double *iv2, double *iv1,
                                double *fw00, double *fw11, double *fw22, double *val,
                                double *gvec, double *hess, const int *needD);
...
GAGE_EXPORT void gageScl3PFilterN(gageShape *shape, int fd, double *iv3, double *iv2,
                                  double *iv1, double *fw00, double *fw11, double *fw22,
...
GAGE_EXPORT void gageShapeWtoI(const gageShape *shape, double index[3],
                               const double world[3]);
GAGE_EXPORT void gageShapeItoW(const gageShape *shape, double world[3],
...
/* optimsig.c */
GAGE_EXPORT int gageOptimSigSet(double *scale, unsigned int num, unsigned int sigmaMax);
GAGE_EXPORT gageOptimSigContext *gageOptimSigContextNew(unsigned int dim,
                                                        unsigned int sampleNumMax,
                                                        unsigned int trueImgNum,
                                                        double sigmaMin, double sigmaMax,
                                                        double cutoff);
GAGE_EXPORT gageOptimSigContext *gageOptimSigContextNix(gageOptimSigContext *oscx);
GAGE_EXPORT int gageOptimSigCalculate(gageOptimSigContext *oscx,
                                      /* output */ double *sigma, unsigned int sigmaNum,
                                      const NrrdKernelSpec *kssSpec, int imgMeasr,
                                      int allMeasr, unsigned int maxIter,
                                      double convEps);
...
GAGE_EXPORT double gageStackWtoI(gageContext *ctx, double swrl, int *outside);
GAGE_EXPORT double gageStackItoW(gageContext *ctx, double si, int *outside);
...
GAGE_EXPORT int gageStackPerVolumeAttach(gageContext *ctx,
                                         ...
                                         unsigned int blnum);
GAGE_EXPORT int gageStackProbe(gageContext *ctx, double xi, double yi, double zi,
                               double si);
GAGE_EXPORT int gageStackProbeSpace(gageContext *ctx, double x, double y, double z,
                                    double s, int indexSpace, int clamp);
...
GAGE_EXPORT int gageStackBlurParmRenormalizeSet(gageStackBlurParm *sbp, int renormalize);
GAGE_EXPORT int gageStackBlurParmDgGoodSigmaMaxSet(gageStackBlurParm *sbp,
                                                   double dgGoodSigmaMax);
GAGE_EXPORT int gageStackBlurParmBoundarySet(gageStackBlurParm *sbp, int boundary,
                                             double padValue);
GAGE_EXPORT int gageStackBlurParmBoundarySpecSet(gageStackBlurParm *sbp,
                                                 const NrrdBoundarySpec *bsp);
GAGE_EXPORT int gageStackBlurParmNeedSpatialBlurSet(gageStackBlurParm *sbp, int sblur);
GAGE_EXPORT int gageStackBlurParmVerboseSet(gageStackBlurParm *sbp, int verbose);
GAGE_EXPORT int gageStackBlurParmOneDimSet(gageStackBlurParm *sbp, int oneDim);
...
GAGE_EXPORT int gageStackBlurCheck(const Nrrd *const nblur[], gageStackBlurParm *sbp,
                                   const Nrrd *nin, const gageKind *kind);
GAGE_EXPORT int gageStackBlurGet(Nrrd *const nblur[], int *recomputedP,
                                 gageStackBlurParm *sbp, const char *format,
                                 const Nrrd *nin, const gageKind *kind);
...
GAGE_EXPORT int gageKernelSet(gageContext *ctx, int which, const NrrdKernel *k,
                              const double *kparm);
...
/* st.c */
GAGE_EXPORT int gageStructureTensor(Nrrd *nout, const Nrrd *nin, int dScale, int iScale,
                                    int dsmp);

/* deconvolve.c */
GAGE_EXPORT int gageDeconvolve(Nrrd *nout, double *lastDiffP, const Nrrd *nin,
                               const gageKind *kind, const NrrdKernelSpec *ksp,
                               int typeOut, unsigned int maxIter, int saveAnyway,
