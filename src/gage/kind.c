...
gageKindCheck(const gageKind *kind) {
  ...
  if (kind->itemMax > GAGE_ITEM_MAX) {
    biffAddf(GAGE, "%s: kind \"%s\" item max %d > GAGE_ITEM_MAX %d", me, kind->name,
             kind->itemMax, GAGE_ITEM_MAX);
    return 1;
  }
  for (ii = 1; ii <= kind->itemMax; ii++) {
    item = kind->table + ii;
    if (ii != item->enumVal) {
      biffAddf(GAGE, "%s: \"%s\"-kind \"%s\" (item %d) has enumVal %d (not %d)", me,
               kind->name, airEnumStr(kind->enm, ii), ii, item->enumVal, ii);
      return 1;
    }
      ...
      if (!(0 <= item->prereq[pi])) {
        if (kind->dynamicAlloc) {
          biffAddf(GAGE,
                   "%s: (dynamic) \"%s\"-kind \"%s\" (item %d) "
                   "prereq %d (%d) not set?",
                   me, kind->name, airEnumStr(kind->enm, ii), ii, pi, item->prereq[pi]);
        } else {
          biffAddf(GAGE,
                   "%s: \"%s\"-kind \"%s\" (item %d) has invalid "
                   "prereq %d (%d)",
    ...
    if (0 != pitem) {
      if (1 == ii) {
        biffAddf(GAGE,
                 "%s: first item (index 1) of \"%s\"-kind can't "
      ...
      if (!(AIR_IN_CL(1, pitem, kind->itemMax))) {
        biffAddf(GAGE,
                 "%s: item %d of \"%s\"-kind wants parent item %d "
...
gageKindTotalAnswerLength(const gageKind *kind) {
  ...
  alen = 0;
  for (ii = 1; ii <= kind->itemMax; ii++) {
    alen += (0 == kind->table[ii].parentItem ? kind->table[ii].answerLength : 0);
  }
  return alen;
}
...
_gageKindAnswerOffset(const gageKind *kind, int item) {
  ...
  if (0 != parent) {
    /* we're a sub-item */
    return (kind->table[item].parentIndex + _gageKindAnswerOffset(kind, parent));
  }
...
gageKindVolumeCheck(const gageKind *kind, const Nrrd *nrrd) {
  ...
  if (!(nrrd->dim == 3 + kind->baseDim)) {
    biffAddf(GAGE, "%s: nrrd should be %u-D, not %u-D", me, 3 + kind->baseDim,
             nrrd->dim);
    return 1;
  }
  ...
  if (kind->baseDim) {
    ...
    if (1 == kind->baseDim) {
      ...
      if (kind->valLen != numsub) {
        biffAddf(GAGE,
                 "%s: %s kind needs %u values below baseDim axis %u, "
                 "not %s",
                 me, kind->name, kind->valLen, kind->baseDim,
