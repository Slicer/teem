...
#undef CHECK
  ...
  if (nrrdKernelSpecCompare(aa->kspec, bb->kspec, differ, subexplain)) {
  ...
  if (nrrdBoundarySpecCompare(aa->bspec, bb->bspec, differ, subexplain)) {
...
gageStackBlurParmCopy(gageStackBlurParm *dst, const gageStackBlurParm *src) {
  ...
  if (gageStackBlurParmSigmaSet(dst, src->num, src->sigmaRange[0], src->sigmaRange[1],
                                src->sigmaSampling)
  ...
  if (gageStackBlurParmCompare(dst, "copy", src, "original", &differ, explain)) {
...
gageStackBlurParmSigmaSet(gageStackBlurParm *sbp, unsigned int num, double sigmaMin,
  ...
  if (!(sigmaMin < sigmaMax)) {
    biffAddf(GAGE, "%s: need sigmaMax %g > sigmaMin %g", me, sigmaMax, sigmaMin);
    return 1;
  }
  if (airEnumValCheck(gageSigmaSampling, sigmaSampling)) {
    biffAddf(GAGE, "%s: %d is not a valid %s", me, sigmaSampling,
    ...
    for (ii = 0; ii < num; ii++) {
      if (ii) {
        fprintf(stderr,
                "%s:           "
                "| deltas: %g\t               %g\n",
                me, sbp->sigma[ii] - sbp->sigma[ii - 1],
                gageTauOfSig(sbp->sigma[ii]) - gageTauOfSig(sbp->sigma[ii - 1]));
      }
      fprintf(stderr, "%s: sigma[%02u]=%g%s\t         tau=%g\n", me, ii, sbp->sigma[ii],
              !sbp->sigma[ii] ? "     " : "", gageTauOfSig(sbp->sigma[ii]));
    }
  }
...
gageStackBlurParmScaleSet(gageStackBlurParm *sbp, unsigned int num, double smin,
  ...
  fprintf(stderr,
          ...
          me);
  if (uniform && optimal) {
    biffAddf(GAGE, "%s: can't have both uniform and optimal sigma sampling", me);
    return 1;
  }
  sampling = (uniform ? gageSigmaSamplingUniformSigma
                      : (optimal ? gageSigmaSamplingOptimal3DL2L2
...
gageStackBlurParmCheck(const gageStackBlurParm *sbp) {
  ...
  for (ii = 0; ii < sbp->num; ii++) {
    if (!AIR_EXISTS(sbp->sigma[ii])) {
      biffAddf(GAGE, "%s: sigma[%u] = %g doesn't exist", me, ii, sbp->sigma[ii]);
      return 1;
    }
    if (ii) {
      if (!(sbp->sigma[ii - 1] < sbp->sigma[ii])) {
        biffAddf(GAGE, "%s: sigma[%u] = %g not < sigma[%u] = %g", me, ii,
                 sbp->sigma[ii - 1], ii + 1, sbp->sigma[ii]);
        return 1;
      }
    }
  }
...
gageStackBlurParmParse(gageStackBlurParm *sbp,
  ...
  if (!(3 == airStrntok(mnmfS, "-") || 4 == airStrntok(mnmfS, "-"))) {
    biffAddf(GAGE, "%s: didn't get 3 or 4 \"-\"-separated tokens in \"%s\"", me, mnmfS);
    airMopError(mop);
    return 1;
  }
  ...
  if (haveFlags) {
      ...
      */
      if (strchr("1ruop", *ff)) {
        ...
        if (extraFlags) {
          ...
          biffAddf(GAGE, "%s: got extra flag '%c' but NULL extraFlag", me, *ff);
          airMopError(mop);
          return 1;
        }
      }
    ...
    if (ff && '+' == *ff) {
      biffAddf(GAGE,
               "%s: sorry, can no longer indicate a derivative "
               "normalization bias via '+' in \"%s\" in flags \"%s\"; "
               "use \"dnbias=\" parm instead",
               me, ff, flags);
      airMopError(mop);
      return 1;
    }
    ...
    for (parmIdx = 0; parmIdx < parmNum; parmIdx++) {
      ...
      if (strcpy(xeq, "k=") && stok == strstr(stok, xeq)) {
        ...
        if (nrrdKernelSpecParse(kspec, pval)) {
          biffMovef(GAGE, NRRD, "%s: couldn't parse \"%s\" as blurring kernel", me,
                    pval);
          airMopError(mop);
          return 1;
        }
        ...
        if (nrrdBoundarySpecParse(bspec, pval)) {
          biffMovef(GAGE, NRRD, "%s: couldn't parse \"%s\" as boundary", me, pval);
          airMopError(mop);
          return 1;
        }
        ...
        if (1 != sscanf(pval, "%lg", &dggsm)) {
          biffAddf(GAGE, "%s: couldn't parse \"%s\" as dgGoodSigmaMax double", me, pval);
          airMopError(mop);
          return 1;
        }
        dggsmGot = AIR_TRUE;
        ...
        if (extraParmsP) {
          ...
          biffAddf(GAGE, "%s: got extra parm \"%s\" but NULL extraParmsP", me, stok);
          airMopError(mop);
          return 1;
        }
      }
  ...
  /* setting sbp fields */
  E = 0;
  if (!E) E |= gageStackBlurParmSigmaSet(sbp, sigmaNum, sigmaMin, sigmaMax, sampling);
  if (kspec) {
...
gageStackBlurParmSprint(char str[AIR_STRLEN_LARGE + 1],
  ...
  out = str;
  sprintf(out, "%.17g-%u-%.17g", sbp->sigmaRange[0], sbp->num, sbp->sigmaRange[1]);
  out += strlen(out);
  hef = AIR_FALSE;
  if (extraFlag) {
    for (fi = 0; fi < 256; fi++) {
  ...
  needFlags = (sbp->oneDim || sbp->renormalize || sbp->needSpatialBlur || hef);
  if (needFlags) {
    strcat(out, "-");
    if (sbp->oneDim) {
      strcat(out, "1");
  ...
  if (sbp->kspec && nrrdKernelDiscreteGaussian == sbp->kspec->kernel
...
_blurValAlloc(airArray *mop, gageStackBlurParm *sbp, NrrdKernelSpec *kssb,
  ...
  for (blIdx = 0; blIdx < sbp->num; blIdx++) {
    ...
    sprintf(blurVal[blIdx].val[6], "%s", sbp->oneDim ? "true" : "false");
    sprintf(blurVal[blIdx].val[7], "%s", spatialBlurred ? "true" : "false");
...
_stackBlurDiscreteGaussFFT(Nrrd *const nblur[], gageStackBlurParm *sbp, const Nrrd *nin,
  ...
  double (*lup)(const void *, size_t), (*ins)(void *, size_t, double), *ww[3], tblur,
    theta, *inFT, *outFT;
  ...
  if (nrrdAxisInfoCopy(ninC, nin, axmap, NRRD_AXIS_INFO_NONE)
      || nrrdBasicInfoCopy(ninC, nin,
                           (NRRD_BASIC_INFO_DATA_BIT | NRRD_BASIC_INFO_DIMENSION_BIT
  ...
  */
  ...
  if (nrrdFFT(ninFT, ninC, ftaxes, 3, +1 /* forward */, AIR_TRUE /* rescale */,
  ...
  */
  ...
  for (blIdx = 0; blIdx < sbp->num; blIdx++) {
    if (sbp->verbose) {
      fprintf(stderr, "%s: . . . %u/%u (scale %g, tau %g) . . . ", me, blIdx, sbp->num,
              sbp->sigma[blIdx], gageTauOfSig(sbp->sigma[blIdx]));
      fflush(stderr);
    }
    ...
    */
    ...
    if (nrrdFFT(noutCd, noutFT, ftaxes, 3, -1 /* backward */, AIR_TRUE /* rescale */,
                nrrdFFTWPlanRigorEstimate /* should generalize! */)
        || (nrrdTypeDouble == nin->type
              ? nrrdCopy(noutC, noutCd)
              : nrrdCastClampRound(noutC, noutCd, nin->type, AIR_TRUE /* clamp */,
...
_stackBlurSpatial(Nrrd *const nblur[], gageStackBlurParm *sbp, NrrdKernelSpec *kssb,
  ...
  for (axi = 0; axi < 3; axi++) {
    if (!E)
      E |= nrrdResampleSamplesSet(rsmc, kind->baseDim + axi,
  ...
  for (blIdx = 0; blIdx < sbp->num; blIdx++) {
    ...
    if (iterative) {
      ...
      if (sbp->verbose) {
        fprintf(stderr, "\n");
        fprintf(stderr,
                "%s: scale %g == time %g (tau %g);\n"
                "               timeLeft %g = %g - %g\n",
                me, sbp->sigma[blIdx], timeNow, gageTauOfTee(timeNow), timeLeft, timeNow,
                timeDone);
      ...
      do {
        ...
        timeDo = (timeLeft > timeStepMax ? timeStepMax : timeLeft);
        ...
        for (axi = 0; axi < 3; axi++) {
          if (!sbp->oneDim || !axi) {
            /* we set the blurring kernel on this axis if
               ...
               but this is axi == 0 */
            if (!E)
              E |= nrrdResampleKernelSet(rsmc, kind->baseDim + axi, kssb->kernel,
                                         kssb->parm);
          } else {
            ...
            if (!E)
              E |= nrrdResampleKernelSet(rsmc, kind->baseDim + axi, nrrdKernelBox,
                                         boxparm);
          }
        }
        if (sbp->verbose) {
      ...
      if (!E)
        E |= nrrdCastClampRound(nblur[blIdx], niter, nin->type, AIR_TRUE,
    ...
    if (E) {
      ...
      biffMovef(GAGE, NRRD, "%s: trouble w/ %u of %u (scale %g)", me, blIdx, sbp->num,
                sbp->sigma[blIdx]);
      airMopError(mop);
      return 1;
    }
...
gageStackBlur(Nrrd *const nblur[], gageStackBlurParm *sbp, const Nrrd *nin,
  ...
  fftable = (!sbp->needSpatialBlur && nrrdBoundaryWrap == sbp->bspec->boundary
  ...
  if (fftable && nrrdFFTWEnabled) {
    ...
    if (fftable && !nrrdFFTWEnabled) {
      if (sbp->verbose) {
        fprintf(stderr,
                "%s: NOTE: FFT-based blurring applicable but not "
      ...
      if (sbp->verbose) {
        ...
        fprintf(stderr,
                "%s: (FFT-based blurring not applicable: "
                "need spatial blur=%s, boundary=%s, kernel=%s)\n",
                me, sbp->needSpatialBlur ? "yes" : "no", bstr, kstr);
      }
    }
...
gageStackBlurCheck(const Nrrd *const nblur[], gageStackBlurParm *sbp, const Nrrd *nin,
  ...
  if (gageStackBlurParmCheck(sbp)
      ...
      || (!(blurVal = _blurValAlloc(mop, sbp, kssb, nin,
                                    (sbp->needSpatialBlur ? AIR_TRUE : AIR_FALSE))))) {
  ...
  for (blIdx = 0; blIdx < sbp->num; blIdx++) {
    if (nin->type != nblur[blIdx]->type) {
      biffAddf(GAGE, "%s: nblur[%u]->type %s != nin type %s\n", me, blIdx,
               airEnumStr(nrrdType, nblur[blIdx]->type),
    ...
    if (gageShapeSet(shapeOld, nblur[blIdx], kind->baseDim)
      ...
      biffAddf(GAGE, "%s: trouble, or nblur[%u] shape != nin shape", me, blIdx);
      airMopError(mop);
      return 1;
    }
    ...
    for (kvpIdx = 0; kvpIdx < KVP_NUM; kvpIdx++) {
      ...
      if (KVP_DGGSM_IDX != kvpIdx) {
        if (!tmpval) {
          biffAddf(GAGE, "%s: didn't see key \"%s\" in nblur[%u]", me, _blurKey[kvpIdx],
                   blIdx);
          airMopError(mop);
          return 1;
        }
...
gageStackBlurGet(Nrrd *const nblur[], int *recomputedP, gageStackBlurParm *sbp,
  ...
  if (!airStrlen(format)) {
    ...
    if (!firstExists) {
      ...
      if (sbp->verbose) {
        fprintf(stderr,
                "%s: will recompute blurrings that couldn't be "
                "read:\n%s\n",
                me, suberr);
      }
      recompute = AIR_TRUE;
    } else if (gageStackBlurCheck(AIR_CAST(const Nrrd *const *, nblur), sbp, nin,
                                  kind)) {
      airMopAdd(mop, suberr = biffGetDone(GAGE), airFree, airMopAlways);
      if (sbp->verbose) {
        fprintf(stderr,
                "%s: will recompute blurrings (from \"%s\") "
                "that don't match:\n%s\n",
                me, format, suberr);
      }
      recompute = AIR_TRUE;
...
gageStackBlurManage(Nrrd ***nblurP, int *recomputedP, gageStackBlurParm *sbp,
  ...
  if (recomputed && format && saveIfComputed) {
    ...
    if (enc) {
      if (!enc->available()) {
        biffAddf(GAGE,
                 "%s: requested %s encoding which is not "
    ...
    if (!E)
      E |= nrrdSaveMulti(format, AIR_CAST(const Nrrd *const *, nblur), sbp->num, 0, nio);
    if (E) {
