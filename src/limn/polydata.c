...
_limnPolyDataInfoAlloc(limnPolyData *pld, unsigned int infoBitFlag,
  ...
  if (vertNum != pld->rgbaNum && ((1 << limnPolyDataInfoRGBA) & infoBitFlag)) {
  ...
  if (vertNum != pld->normNum && ((1 << limnPolyDataInfoNorm) & infoBitFlag)) {
  ...
  if (vertNum != pld->tex2Num && ((1 << limnPolyDataInfoTex2) & infoBitFlag)) {
  ...
  if (vertNum != pld->tangNum && ((1 << limnPolyDataInfoTang) & infoBitFlag)) {
...
limnPolyDataCopy(limnPolyData *pldB, const limnPolyData *pldA) {
  ...
  if (limnPolyDataAlloc(pldB, limnPolyDataInfoBitFlag(pldA), pldA->xyzwNum,
                        pldA->indxNum, pldA->primNum)) {
...
limnPolyDataPrimitiveArea(Nrrd *nout, limnPolyData *pld) {
  ...
  for (primIdx = 0; primIdx < pld->primNum; primIdx++) {
    ...
    case limnPrimitiveTriangleFan:
    case limnPrimitiveQuads:
      biffAddf(LIMN, "%s: sorry, haven't implemented area(prim[%u]=%s) yet", me, primIdx,
               airEnumStr(limnPrimitive, pld->type[primIdx]));
      return 1;
      break;
...
limnPolyDataRasterize(Nrrd *nout, limnPolyData *pld, double min[3], double max[3],
  ...
  if (nrrdTypeBlock == type) {
    biffAddf(LIMN, "%s: can't use output type %s", me, airEnumStr(nrrdType, type));
    return 1;
  }
  if (!(min[0] < max[0] && min[1] < max[1] && min[2] < max[2])) {
    biffAddf(LIMN, "%s min (%g,%g,%g) not < max (%g,%g,%g)", me, min[0], min[1], min[2],
             max[0], max[1], max[2]);
    return 1;
  }
  ...
  for (vertIdx = 0; vertIdx < pld->xyzwNum; vertIdx++) {
    ...
    if (!(AIR_IN_OP(min[0], xyz[0], max[0]) && AIR_IN_OP(min[1], xyz[1], max[1])
...
void
limnPolyDataColorSet(limnPolyData *pld, unsigned char RR, unsigned char GG,
