...
*/
static unsigned int
flipListIntx(unsigned int *dstC, const unsigned int *_srcA, const unsigned int *_srcB) {
...
*/
static int
flipNeed(Nrrd *nVertWithTri, unsigned int triIdx, unsigned int vertA,
         unsigned int vertB) {
...
neighborsCheckPush(Nrrd *nTriWithVert, Nrrd *nVertWithTri, unsigned char *triDone,
  ...
  vertWithTri = AIR_CAST(unsigned int *, nVertWithTri->data);
  flipNeighborsGet(nTriWithVert, nVertWithTri, neighGot, neighInfo, intxBuff, triIdx);
  /*
  for (ii=0; ii<3; ii++) {
    ...
    if (neighGot[ii]) {
      ...
      if (!splitting) {
        if (!triDone[neighInfo[ii][0]]) {
          ...
          need = flipNeed(nVertWithTri, neighInfo[ii][0], neighInfo[ii][1],
                          neighInfo[ii][2]);
          if (need) {
        ...
        if (need && triDone[neighInfo[ii][0]]) {
          ...
          for (splitIdx = 0; splitIdx < splitNum; splitIdx++) {
            if (split[2 + 5 * splitIdx] == vert0 && split[3 + 5 * splitIdx] == vert1) {
              break;
            }
          }
...
triangleWithVertex(Nrrd *nTriWithVert, limnPolyData *pld) {
  ...
  unsigned int *triWithVertNum, /* vert ii has triWithVertNum[ii] tris */
    *triWithVert, baseVertIdx, primIdx, vertIdx, maxTriPerVert, totTriIdx;
  airArray *mop;
  ...
  triWithVertNum = AIR_CAST(unsigned int *, calloc(pld->xyzwNum, sizeof(unsigned int)));
  if (!triWithVertNum) {
...
splitListExtract(unsigned int *listLenP, airArray *edgeArr, unsigned char *hitCount,
  ...
  unsigned int *edgeData, edgeNum, *edgeLine, edgeIdx, edgeTmp[5], tmp, nextVertIdx,
    listLen;
...
splitTriSweep(unsigned int *sweep, unsigned int triStart, unsigned int vertPivot,
  ...
  */
  ...
  do {
    ...
    */
    triCurr = sweepTriNext(triLine, vertPivot, vertNext, triCurr, nVertWithTri);
    /*
    fprintf(stderr, "!%s:       triNext(%u,%u) = %u\n", me,
            vertPivot, vertNext, triCurr);
    */
    vertLast = vertNext;
  } while (!(UINT_MAX == triCurr || triStart == triCurr || triStop0 == triCurr
             || triStop1 == triCurr));
  if (!(UINT_MAX == triCurr)) {
    sweep[sweepLen++] = triCurr;
    /*
...
splitTriTrack(unsigned int *track0, unsigned int *track0LenP, unsigned int *track1,
              ...
              unsigned int listLen, int looping) {
  ...
  for (edgeIdx = 0; edgeIdx < (looping ? listLen - 1 : listLen); edgeIdx++) {
    ...
    if (doBack0) {
      sweepLen = splitTriSweep(sweep, edgeLine[0], edgeLine[2], edgeLine[3], stop0,
                               stop1, nTriWithVert, nVertWithTri);
    ...
    if (doBack1) {
      sweepLen = splitTriSweep(sweep, edgeLine[1], edgeLine[2], edgeLine[3], stop0,
                               stop1, nTriWithVert, nVertWithTri);
    ...
    sweepLen = splitTriSweep(sweep, edgeLine[0], edgeLine[3], edgeLine[2], stop0, stop1,
                             nTriWithVert, nVertWithTri);
    for (triIdx = 0; triIdx < sweepLen; triIdx++) {
      track0[len0++] = sweep[triIdx];
    }
    sweepLen = splitTriSweep(sweep, edgeLine[1], edgeLine[3], edgeLine[2], stop0, stop1,
                             nTriWithVert, nVertWithTri);
    for (triIdx = 0; triIdx < sweepLen; triIdx++) {
    ...
    if (edgeIdx < listLen - 1) {
      ...
      if (track0[len0 - 1] == nextLine[0] && track1[len1 - 1] == nextLine[1]) {
        /* fprintf(stderr, "!%s: tracking went 0->0, 1->1\n", me); */
        doBack0 = doBack1 = AIR_FALSE;
      } else if (track0[len0 - 1] == nextLine[1] && track1[len1 - 1] == nextLine[0]) {
        ...
        biffAddf(LIMN, "%s: edge %u/%u, sweep ends %u,%u != want %u,%u", me, edgeIdx,
                 listLen, track0[len0 - 1], track1[len1 - 1], nextLine[0], nextLine[1]);
        return 1;
      }
    } else {
      doBack0 = doBack1 = AIR_FALSE;
...
splitVertDup(limnPolyData *pld, airArray *edgeArr, unsigned int edgeDoneNum,
  ...
  unsigned int *vixLut, ii, vixLutLen, oldVertNum, newVertNum, *edgeData, bitflag,
    trackIdx, vert0, vert1;
  airArray *mop;
  limnPolyData pldTmp;
  ...
  if (limnPolyDataAlloc(pld, bitflag, newVertNum, pld->indxNum, pld->primNum)) {
  ...
  vixLut = AIR_CAST(unsigned int *, calloc(2 * vixLutLen, sizeof(unsigned int)));
  ...
  for (ii = 0; ii < vixLutLen; ii++) {
    ELL_4V_COPY(pld->xyzw + 4 * vixLut[1 + 2 * ii], pld->xyzw + 4 * vixLut[0 + 2 * ii]);
    if ((1 << limnPolyDataInfoRGBA) & bitflag) {
...
doSplitting(limnPolyData *pld, Nrrd *nTriWithVert, Nrrd *nVertWithTri,
  ...
  unsigned int edgeIdx, *edgeData, *edgeLine = NULL, vertIdx, vertNum, splitNum,
                                   edgeDoneNum, listLen = 0, *track0, track0Len = 0,
                                   *track1, *sweep, track1Len = 0, maxTriPerVert;
  ...
  vertNum = pld->xyzwNum;
  hitCount = AIR_CAST(unsigned char *, calloc(vertNum, sizeof(unsigned char)));
  maxTriPerVert = AIR_UINT(nTriWithVert->axis[0].size - 1);
  track0 = AIR_CAST(unsigned int *,
                    calloc(maxTriPerVert * edgeArr->len, sizeof(unsigned int)));
  track1 = AIR_CAST(unsigned int *,
                    calloc(maxTriPerVert * edgeArr->len, sizeof(unsigned int)));
  sweep = AIR_CAST(unsigned int *, calloc(maxTriPerVert, sizeof(unsigned int)));
  ...
  for (edgeIdx = 0; edgeIdx < edgeArr->len; edgeIdx++) {
    ...
    if (ha > 2 || hb > 2) {
      biffAddf(LIMN, "%s: edge %u (vert %u %u) created hit counts %u %u", me, edgeIdx,
               edgeLine[2], edgeLine[3], ha, hb);
      airMopError(mop);
      return 1;
    }
  }
  ...
  */
  for (passIdx = 0; passIdx < 2; passIdx++) {
    ...
    while (vertIdx < vertNum) {
      ...
      */
      if (!E)
        E |= splitTriTrack(track0, &track0Len, track1, &track1Len, sweep, nTriWithVert,
                           nVertWithTri, edgeArr, edgeDoneNum, listLen, passIdx);
      /* ,,,,,,,,,,,,,,,,,,,,,
      if (!E) {
      ...
      if (!E)
        E |= splitVertDup(pld, edgeArr, edgeDoneNum, listLen, track0, track0Len,
                          passIdx);
      if (E) {
        biffAddf(LIMN, "%s: trouble on split %u (done %u/%u)", me, splitNum, edgeDoneNum,
                 AIR_UINT(edgeArr->len));
        return 1;
      }
      edgeDoneNum += listLen;
      /*
...
_limnPolyDataVertexWindingProcess(limnPolyData *pld, int splitting) {
  ...
  unsigned char *triDone; /* 1D array (len totTriNum) record of done-ness */
  ...
  */
  ...
  mop = airMopNew();
  triDone = AIR_CAST(unsigned char *, calloc(totTriNum, sizeof(unsigned char)));
  ...
  if (triangleWithVertex(nTriWithVert, pld) || vertexWithTriangle(nVertWithTri, pld)) {
  ...
  maxTriPerVert = AIR_UINT(nTriWithVert->axis[0].size - 1);
  intxBuff = AIR_CAST(unsigned int *, calloc(maxTriPerVert, sizeof(unsigned int)));
  if (!intxBuff) {
  ...
  */
  ...
  appu.ui = &okay;
  okayArr = airArrayNew(appu.v, NULL, sizeof(unsigned int), maxTriPerPrim);
  airMopAdd(mop, okayArr, (airMopper)airArrayNuke, airMopAlways);
  if (splitting) {
    appu.ui = &split;
    splitArr = airArrayNew(appu.v, NULL, 5 * sizeof(unsigned int), maxTriPerPrim);
  ...
  */
  while (doneTriNum < trueTriNum) {
    ...
    */
    doneTriNum += neighborsCheckPush(nTriWithVert, nVertWithTri, triDone, okayArr,
                                     intxBuff, splitArr, totTriIdx, splitting);
    while (okayArr->len) {
      unsigned int popped;
      ...
      */
      doneTriNum += neighborsCheckPush(nTriWithVert, nVertWithTri, triDone, okayArr,
                                       intxBuff, splitArr, popped, splitting);
    }
  }
...
limnPolyDataCCFind(limnPolyData *pld) {
  ...
  unsigned int realTriNum, *triMap, *triWithVert, vertIdx, *indxOld, *indxNew,
    primNumOld, *icntOld, *icntNew, *baseIndx, primIdxNew, primNumNew, passIdx,
    eqvNum = 0;
  ...
  if (1 != primNumOld) {
    biffAddf(LIMN,
             "%s: sorry! stupid implementation can't "
             "do primNum %u (only 1)",
             me, primNumOld);
    airMopError(mop);
    return 1;
  ...
  typeOld = pld->type;
  icntOld = pld->icnt;
  indxNew = AIR_CAST(unsigned int *, calloc(pld->indxNum, sizeof(unsigned int)));
  typeNew = AIR_CAST(unsigned char *, calloc(primNumNew, sizeof(unsigned char)));
  icntNew = AIR_CAST(unsigned int *, calloc(primNumNew, sizeof(unsigned int)));
...
limnPolyDataPrimitiveSort(limnPolyData *pld, const Nrrd *_nval) {
  ...
  if (!(1 == _nval->dim && nrrdTypeBlock != _nval->type
        && _nval->axis[0].size == pld->primNum)) {
    biffAddf(LIMN,
             "%s: need 1-D %u-len scalar nrrd "
             "(not %u-D type %s, axis[0].size %u)",
             me, pld->primNum, _nval->dim, airEnumStr(nrrdType, _nval->type),
  ...
  qsort(rec, pld->primNum, 2 * sizeof(double), nrrdValCompareInv[nrrdTypeDouble]);

  startIndx = AIR_CAST(unsigned int **, calloc(pld->primNum, sizeof(unsigned int *)));
  indxNew = AIR_CAST(unsigned int *, calloc(pld->indxNum, sizeof(unsigned int)));
  icntNew = AIR_CAST(unsigned int *, calloc(pld->primNum, sizeof(unsigned int)));
  typeNew = AIR_CAST(unsigned char *, calloc(pld->primNum, sizeof(unsigned char)));
  ...
  for (primIdx = 0; primIdx < pld->primNum; primIdx++) {
    ...
    memcpy(baseIndx, startIndx[sortIdx], pld->icnt[sortIdx] * sizeof(unsigned int));
...
limnPolyDataPrimitiveSelect(limnPolyData *pldOut,
  ...
  Nrrd *nmask;
  double *mask;
  unsigned int oldBaseVertIdx, oldPrimIdx, oldVertIdx, bitflag, *old2newMap, *new2oldMap,
  ...
  if (!(1 == _nmask->dim && nrrdTypeBlock != _nmask->type
        && _nmask->axis[0].size == pldIn->primNum)) {
    biffAddf(LIMN,
             "%s: need 1-D %u-len scalar nrrd "
             "(not %u-D type %s, axis[0].size %u)",
             me, pldIn->primNum, _nmask->dim, airEnumStr(nrrdType, _nmask->type),
  ...
  old2newMap = AIR_CAST(unsigned int *, calloc(pldIn->xyzwNum, sizeof(unsigned int)));
  airMopAdd(mop, old2newMap, airFree, airMopAlways);
  vertUsed = AIR_CAST(unsigned char *, calloc(pldIn->xyzwNum, sizeof(unsigned char)));
  ...
  new2oldMap = AIR_CAST(unsigned int *, calloc(newVertNum, sizeof(unsigned int)));
 ...
 */
static int
clipEdge(int disc, int kept, Nrrd *nval, double *thresh, int *newIdx, airArray *llistArr,
         limnPolyData *pld, unsigned int bitflag, limnPolyData *newpld,
         airArray *xyzwArr, airArray *rgbaArr, airArray *normArr, airArray *tex2Arr,
         airArray *tangArr) {
  ...
  for (i = 0; i < nk; i++) {
    ...
    if (thisalpha < 1.0 && thisalpha > alpha) alpha = thisalpha;
  }
  /* add interpolated vertex */
  q = airArrayLenIncr(xyzwArr, 1);
  ELL_4V_LERP_TT(newpld->xyzw + 4 * q, float, alpha, pld->xyzw + 4 * disc,
                 pld->xyzw + 4 * kept);
  if ((1 << limnPolyDataInfoRGBA) & bitflag) {
    airArrayLenIncr(rgbaArr, 1);
    ELL_4V_LERP_TT(newpld->rgba + 4 * q, unsigned char, alpha, pld->rgba + 4 * disc,
                   pld->rgba + 4 * kept);
  }
  if ((1 << limnPolyDataInfoNorm) & bitflag) {
    float fnorm[3];
  ...
  if ((1 << limnPolyDataInfoTex2) & bitflag) {
    ...
    ELL_2V_LERP_TT(newpld->tex2 + 2 * q, float, alpha, pld->tex2 + 2 * disc,
                   pld->tex2 + 2 * kept);
  }
  if ((1 << limnPolyDataInfoTang) & bitflag) {
    airArrayLenIncr(tangArr, 1);
    ELL_3V_LERP_TT(newpld->tang + 3 * q, float, alpha, pld->tang + 3 * disc,
                   pld->tang + 3 * kept);
  }
...
limnPolyDataClipMulti(limnPolyData *pld, Nrrd *nval, double *thresh) {
  ...
  airArray *xyzwArr, *rgbaArr = NULL, *normArr = NULL, *tex2Arr = NULL, *tangArr = NULL,
  ...
  if (nval->dim == 1) {
    nk = 1;
    nvert = AIR_UINT(nval->axis[0].size);
  } else if (nval->dim == 2) {
    nk = AIR_UINT(nval->axis[0].size);
    nvert = AIR_UINT(nval->axis[1].size);
  } else {
    biffAddf(LIMN, "%s: need 1D or 2D input array, got %uD", me, nval->dim);
    return 1;
  }
  if (nvert != pld->xyzwNum) {
    biffAddf(LIMN, "%s: # verts %u != # values %u", me, pld->xyzwNum, nvert);
    return 1;
  }
  if ((1 << limnPrimitiveTriangles) != limnPolyDataPrimitiveTypes(pld)) {
    biffAddf(LIMN, "%s: sorry, can only handle %s primitives", me,
  ...
  E = AIR_FALSE;
  if (!E) {
    E |= !(keepVert = AIR_CAST(unsigned char *, calloc(pld->xyzwNum, sizeof(char))));
  }
  if (!E) {
  ...
  if (!E) {
    ...
    appu.f = &(newpld->xyzw);
    E |= !(xyzwArr = airArrayNew(appu.v, &(newpld->xyzwNum), 4 * sizeof(float), incr));
    if (!E) {
    ...
    if (!E && (1 << limnPolyDataInfoRGBA) & bitflag) {
      appu.uc = &(newpld->rgba);
      E |= !(rgbaArr = airArrayNew(appu.v, &(newpld->rgbaNum), 4 * sizeof(unsigned char),
                                   incr));
      if (!E) {
    ...
    if (!E && (1 << limnPolyDataInfoNorm) & bitflag) {
      appu.f = &(newpld->norm);
      E |= !(normArr = airArrayNew(appu.v, &(newpld->normNum), 3 * sizeof(float), incr));
      if (!E) {
    ...
    if (!E && (1 << limnPolyDataInfoTex2) & bitflag) {
      appu.f = &(newpld->tex2);
      E |= !(tex2Arr = airArrayNew(appu.v, &(newpld->tex2Num), 2 * sizeof(float), incr));
      if (!E) {
    ...
    if (!E && (1 << limnPolyDataInfoTang) & bitflag) {
      appu.f = &(newpld->tang);
      E |= !(tangArr = airArrayNew(appu.v, &(newpld->tangNum), 3 * sizeof(float), incr));
      if (!E) {
    ...
    if (!E) {
      ...
      if (incr < 10) incr = 10; /* ...but at least 10. */
      appu.ui = &(newpld->indx);
      E |= !(indxArr = airArrayNew(appu.v, &(newpld->indxNum), sizeof(unsigned int),
                                   incr));
      if (!E) {
    ...
    if (!E) {
      ...
      appu.uc = &(newpld->type);
      E |= !(typeArr = airArrayNew(appu.v, &(newpld->primNum), sizeof(unsigned char),
                                   incr));
      if (!E) {
      ...
      appu.ui = &(newpld->icnt);
      E |= !(icntArr = airArrayNew(appu.v, NULL, sizeof(unsigned int), incr));
      if (!E) {
  ...
  for (i = 0; i < pld->xyzwNum; i++) {
    ...
    for (j = 0; j < nk; j++, idx++) {
      if (lup(nval->data, idx) < thresh[j]) keep = AIR_FALSE;
    }
    if (keep) {
      keepVert[i] = AIR_TRUE;
    }
  ...
  for (i = 0; i < pld->primNum; i++) {
    ...
    for (j = 0; j < oldTriNum; j++, idx += 3) {
        ...
        for (k = 0; k < 3; k++) {
          if (keepVert[pld->indx[idx + k]])
            ...
            newpld->indx[p + k] = clipEdge(pld->indx[idx + k], kept, nval, thresh,
                                           ...
                                           xyzwArr, rgbaArr, normArr, tex2Arr, tangArr);
        }
        break;
      case 2: /* result of clipping is a quad, triangulate */
        ...
        for (k = 0; k < 3; k++) {
          if (keepVert[pld->indx[idx + k]])
            quad[p++] = newIdx[pld->indx[idx + k]];
          else {
            quad[p++] = clipEdge(pld->indx[idx + k], pld->indx[idx + (disck + 2) % 3],
                                 nval, thresh, newIdx, llistArr, pld, bitflag, newpld,
                                 xyzwArr, rgbaArr, normArr, tex2Arr, tangArr);
...
limnPolyDataCompress(const limnPolyData *pld) {
  ...
  if (NULL == (ret = limnPolyDataNew())
      || 0 != limnPolyDataAlloc(ret, infoBitFlag, vertNum, used_indxNum, pld->primNum)) {
...
limnPolyDataJoin(const limnPolyData **plds, unsigned int num) {
  ...
  unsigned int infoBitFlag
    = (1 << limnPolyDataInfoRGBA) | (1 << limnPolyDataInfoNorm)
  ...
  for (i = 0; i < num; i++) {
    ...
    memcpy(ret->xyzw + 4 * vertNum, plds[i]->xyzw, sizeof(float) * 4 * plds[i]->xyzwNum);
    if (ret->rgba != NULL) {
...
limnPolyDataEdgeHalve(limnPolyData *pldOut, const limnPolyData *pldIn) {
  ...
  for (vlo = 0; vlo < nvold; vlo++) {
    ...
    for (vhi = vlo + 1; vhi < nvold; vhi++) {
      ...
      ELL_4V_LERP(pldOut->xyzw + 4 * mid, 0.5f, pldIn->xyzw + 4 * vlo,
                  pldIn->xyzw + 4 * vhi);
      if ((1 << limnPolyDataInfoRGBA) & bitflag) {
        ELL_4V_LERP_TT(pldOut->rgba + 4 * mid, unsigned char, 0.5f,
                       pldIn->rgba + 4 * vlo, pldIn->rgba + 4 * vhi);
      }
      if ((1 << limnPolyDataInfoNorm) & bitflag) {
        float tmp;
        ELL_3V_LERP(pldOut->norm + 3 * mid, 0.5f, pldIn->norm + 3 * vlo,
                    pldIn->norm + 3 * vhi);
        ELL_3V_NORM_TT(pldOut->norm + 3 * mid, float, pldOut->norm + 3 * mid, tmp);
      }
      if ((1 << limnPolyDataInfoTex2) & bitflag) {
        ELL_2V_LERP(pldOut->tex2 + 2 * mid, 0.5f, pldIn->tex2 + 2 * vlo,
                    pldIn->tex2 + 2 * vhi);
      }
      if ((1 << limnPolyDataInfoTang) & bitflag) {
        float tmp;
        ELL_3V_LERP(pldOut->tang + 3 * mid, 0.5f, pldIn->tang + 3 * vlo,
                    pldIn->tang + 3 * vhi);
        ELL_3V_NORM_TT(pldOut->tang + 3 * mid, float, pldOut->tang + 3 * mid, tmp);
      }
    }
  }
...
static void
registerNeighbor(unsigned int *nblist, size_t *len, unsigned int *maxnb, unsigned int u,
                 unsigned int v) {
  unsigned int idx = nblist[u], pointer = u, depth = 1;
  while (idx != 0) {
    if (nblist[idx] == v) return; /* has already been registered */
    pointer = idx + 1;
    idx = nblist[pointer];
    depth++;
  }
  if (depth > *maxnb) *maxnb = depth;
  /* do the registration */
...
limnPolyDataNeighborList(unsigned int **nblist, size_t *len, unsigned int *maxnb,
  ...
  /* allocate *nblist */
  *nblist = (unsigned int *)malloc(sizeof(unsigned int) * (pld->xyzwNum + 2 * estimate));
  if (*nblist == NULL) {
  ...
  for (i = 0; i < pld->primNum; i++) {
    ...
    case limnPrimitiveTriangles:
      ...
      break;
    case limnPrimitiveTriangleStrip:
      if (pld->icnt[i] > 0) registerNeighbor(*nblist, &last, &m, indx[0], indx[1]);
