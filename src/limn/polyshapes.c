...
limnPolyDataCylinder(limnPolyData *pld, unsigned int infoBitFlag, unsigned int thetaRes,
  ...
  for (blah = 0; blah < (sharpEdge ? 2u : 1u); blah++) {
    for (thetaIdx = 0; thetaIdx < thetaRes; thetaIdx++) {
      ...
      ELL_4V_SET_TT(pld->xyzw + 4 * vertIdx, float, cos(theta), sin(theta), 1, 1);
      /*
  ...
  if ((1 << limnPolyDataInfoNorm) & infoBitFlag) {
    ...
    if (sharpEdge) {
      for (thetaIdx = 0; thetaIdx < thetaRes; thetaIdx++) {
        ...
        cth = cos(theta);
        sth = sin(theta);
        ELL_3V_SET_TT(pld->norm + 3 * (thetaIdx + 0 * thetaRes), float, 0, 0, 1);
        ELL_3V_SET_TT(pld->norm + 3 * (thetaIdx + 1 * thetaRes), float, cth, sth, 0);
        ELL_3V_SET_TT(pld->norm + 3 * (thetaIdx + 2 * thetaRes), float, cth, sth, 0);
        ELL_3V_SET_TT(pld->norm + 3 * (thetaIdx + 3 * thetaRes), float, 0, 0, -1);
      }
    } else {
      for (thetaIdx = 0; thetaIdx < thetaRes; thetaIdx++) {
        theta = AIR_AFFINE(0, thetaIdx, thetaRes, 0, 2 * AIR_PI);
        cth = sq2 * cos(theta);
        sth = sq2 * sin(theta);
        ELL_3V_SET_TT(pld->norm + 3 * (thetaIdx + 0 * thetaRes), float, cth, sth, sq2);
        ELL_3V_SET_TT(pld->norm + 3 * (thetaIdx + 1 * thetaRes), float, cth, sth, -sq2);
      }
    }
  }
...
limnPolyDataCone(limnPolyData *pld, unsigned int infoBitFlag, unsigned int thetaRes,
  ...
  if (sharpEdge) {
    for (thetaIdx = 0; thetaIdx < thetaRes; thetaIdx++) {
      ELL_4V_SET_TT(pld->xyzw + 4 * vertIdx, float, 0, 0, 1, 1);
      ++vertIdx;
    }
  } else {
    ELL_4V_SET_TT(pld->xyzw + 4 * vertIdx, float, 0, 0, 1, 1);
  ...
  if ((1 << limnPolyDataInfoNorm) & infoBitFlag) {
    ...
    if (sharpEdge) {
      for (thetaIdx = 0; thetaIdx < thetaRes; thetaIdx++) {
        ...
        cth = cos(theta);
        sth = sin(theta);
        ELL_3V_SET_TT(pld->norm + 3 * (thetaIdx + 0 * thetaRes), float, cth *isq3,
                      sth *isq3, isq3);
        ELL_3V_SET_TT(pld->norm + 3 * (thetaIdx + 1 * thetaRes), float, cth *isq3,
                      sth *isq3, isq3);
        ELL_3V_SET_TT(pld->norm + 3 * (thetaIdx + 2 * thetaRes), float, 0, 0, -1);
      }
    } else {
      ELL_3V_SET_TT(pld->norm + 3 * (0), float, 0, 0, 1);
      for (thetaIdx = 0; thetaIdx < thetaRes; thetaIdx++) {
        theta = AIR_AFFINE(0, thetaIdx, thetaRes, 0, 2 * AIR_PI);
        cth = cos(theta);
        sth = sin(theta);
        ELL_3V_SET_TT(pld->norm + 3 * (thetaIdx + 1), float, cth *isq3, sth *isq3,
                      -isq3); /* close enough */
      }
    }
  }
...
limnPolyDataSuperquadric(limnPolyData *pld, unsigned int infoBitFlag, float alpha,
  ...
  unsigned int vertIdx, vertNum, fanNum, stripNum, primNum, indxNum, thetaIdx, phiIdx,
    primIdx;
  double theta, phi;
  ...
  for (phiIdx = 1; phiIdx < phiRes; phiIdx++) {
    ...
    for (thetaIdx = 0; thetaIdx < thetaRes; thetaIdx++) {
      ...
      ELL_4V_SET_TT(pld->xyzw + 4 * vertIdx, float,
                    ...
                    airSgnPow(cosp, beta), 1.0);
      if ((1 << limnPolyDataInfoNorm) & infoBitFlag) {
        if (1 == alpha && 1 == beta) {
          ELL_3V_COPY(pld->norm + 3 * vertIdx, pld->xyzw + 4 * vertIdx);
        } else {
          ELL_3V_SET_TT(pld->norm + 3 * vertIdx, float,
                        2 * airSgnPow(cost, 2 - alpha) * airSgnPow(sinp, 2 - beta)
                          / beta,
                        2 * airSgnPow(sint, 2 - alpha) * airSgnPow(sinp, 2 - beta)
                          / beta,
  ...
  for (phiIdx = 0; phiIdx < phiRes; phiIdx++) {
    for (thetaIdx = 0; thetaIdx < thetaRes; thetaIdx++) {
      ...
      if (minRad > 0.0) {
        ...
        rr = sqrt(xx * xx + yy * yy);
        if (rr) {
          (pld->xyzw + 4 * vertIdx)[0] *= AIR_FLOAT(AIR_AFFINE(0, rr, 1, minRad / rr,
                                                               1 / rr));
          (pld->xyzw + 4 * vertIdx)[1] *= AIR_FLOAT(AIR_AFFINE(0, rr, 1, minRad / rr,
                                                               1 / rr));
        }
      }
      if (((1 << limnPolyDataInfoNorm) & infoBitFlag)
...
limnPolyDataSpiralSuperquadric(limnPolyData *pld, unsigned int infoBitFlag, float alpha,
  ...
  if (limnPolyDataSpiralBetterquadric(pld, infoBitFlag, alpha, beta, beta, 0.0, thetaRes,
                                      phiRes)) {
...
limnPolyDataPolarSphere(limnPolyData *pld, unsigned int infoBitFlag,
  ...
  if (limnPolyDataSuperquadric(pld, infoBitFlag, 1.0, 1.0, thetaRes, phiRes)) {
    biffAddf(LIMN, "%s: trouble", me);
    return 1;
  }
  return 0;
...
limnPolyDataSpiralSphere(limnPolyData *pld,
  ...
  if (limnPolyDataSpiralSuperquadric(pld, infoBitFlag, 1.0, 1.0, thetaRes, phiRes)) {
    biffAddf(LIMN, "%s: trouble", me);
    return 1;
  }
  return 0;
...
static float icovertices[36]
  = {0.0f,     ICO_ONE, ICO_TAU, 0.0f,     -ICO_ONE, -ICO_TAU, 0.0f,     -ICO_ONE,
     ICO_TAU,  0.0f,    ICO_ONE, -ICO_TAU, ICO_ONE,  ICO_TAU,  0.0f,     -ICO_ONE,
     -ICO_TAU, 0.0f,    ICO_ONE, -ICO_TAU, 0.0f,     -ICO_ONE, ICO_TAU,  0.0f,
     ICO_TAU,  0.0f,    ICO_ONE, -ICO_TAU, 0.0f,     -ICO_ONE, -ICO_TAU, 0.0f,
     ICO_ONE,  ICO_TAU, 0.0f,    -ICO_ONE};
#undef ICO_ONE
#undef ICO_TAU
...
static unsigned int icofaces[60] = {0,  4,  12, 1,  5,  13, 0,  6,  14, 1,  7,  15,
...
limnPolyDataIcoSphere(limnPolyData *pld, unsigned int infoBitFlag, unsigned int level) {
  ...
  for (i = 0; i < level; ++i) {
    ...
    for (e = 0; e < edgeNum; e += 2) { /* split both edge and anti-edge */
      ...
      ELL_3V_NORM_TT(newverts + 3 * (vertNum + e), float, newverts + 3 * (vertNum + e),
                     norm);
      ELL_3V_SCALE_TT(newverts + 3 * (vertNum + e + 1), float, -1.0,
                      newverts + 3 * (vertNum + e));
    ...
    for (f = 0; f < faceNum; f += 2) { /* split both face and anti-face */
      ...
      char pol11 = 0, pol12 = 0, pol13 = 0, pol21 = 0, pol22 = 0,
           pol23 = 0; /* polarity */
      ...
      newfaces[12 * f + 6] = 2 * oldedge11 - (oldedge11 % 2) + 2
                           - pol11; /* bottom/right */
...
limnPolyDataPlane(limnPolyData *pld, unsigned int infoBitFlag, unsigned int uRes,
  ...
  for (vIdx = 0; vIdx < vRes; vIdx++) {
    ...
    for (uIdx = 0; uIdx < uRes; uIdx++) {
      ...
      if ((1 << limnPolyDataInfoTex2) & infoBitFlag) {
        ELL_2V_SET_TT(pld->tex2 + 2 * vertIdx, float, (uu + 1.0) / 2.0,
                      (vv + 1.0) / 2.0);
      }
      if ((1 << limnPolyDataInfoTang) & infoBitFlag) {
...
limnPolyDataSuperquadric2D(limnPolyData *pld, unsigned int infoBitFlag, float alpha,
  ...
  /* printf("!%s: xyzw[0] = %g %g %g %g\n", me, (pld->xyzw + 4*0)[0], (pld->xyzw +
   * 4*0)[1], (pld->xyzw + 4*0)[2], (pld->xyzw + 4*0)[3]); */
  for (i = 1; i < vertNum; i++) {
    phi = AIR_AFFINE(1, i, vertNum, 0, 2 * AIR_PI);
    ELL_4V_SET_TT(pld->xyzw + 4 * i, float, airSgnPow(cos(phi), alpha),
                  airSgnPow(sin(phi), alpha), 0.0, 1.0);
    /* printf("!%s: xyzw[%u] = %g %g %g %g\n", me, i ,(pld->xyzw + 4*i)[0], (pld->xyzw +
     * 4*i)[1], (pld->xyzw + 4*i)[2], (pld->xyzw + 4*i)[3]); */
  }
  if ((1 << limnPolyDataInfoNorm) & infoBitFlag) {
    for (i = 0; i < vertNum; i++) {
