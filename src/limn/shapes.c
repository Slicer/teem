...
limnObjectCylinderAdd(limnObject *obj, unsigned int lookIdx, unsigned int axis,
  ...
  for (ii = 0; ii <= res - 1; ii++) {
    ...
    switch (axis) {
    case 0:
      tmp = limnObjectVertexAdd(obj, partIdx, 1, AIR_FLOAT(-sin(theta)),
                                AIR_FLOAT(cos(theta)));
      limnObjectVertexAdd(obj, partIdx, -1, AIR_FLOAT(-sin(theta)),
                          AIR_FLOAT(cos(theta)));
      break;
    case 1:
      tmp = limnObjectVertexAdd(obj, partIdx, AIR_FLOAT(sin(theta)), 1,
                                AIR_FLOAT(cos(theta)));
      limnObjectVertexAdd(obj, partIdx, AIR_FLOAT(sin(theta)), -1,
                          AIR_FLOAT(cos(theta)));
      break;
    case 2:
    default:
      tmp = limnObjectVertexAdd(obj, partIdx, AIR_FLOAT(cos(theta)),
                                AIR_FLOAT(sin(theta)), 1);
      limnObjectVertexAdd(obj, partIdx, AIR_FLOAT(cos(theta)), AIR_FLOAT(sin(theta)),
                          -1);
      break;
    }
    if (!ii) {
  ...
  for (ii = 0; ii <= res - 1; ii++) {
    jj = (ii + 1) % res;
    ELL_4V_SET(vII, vII0 + 2 * ii, vII0 + 2 * ii + 1, vII0 + 2 * jj + 1, vII0 + 2 * jj);
    limnObjectFaceAdd(obj, partIdx, lookIdx, 4, vII);
  }
  /* add top */
  for (ii = 0; ii <= res - 1; ii++) {
...
limnObjectConeAdd(limnObject *obj, unsigned int lookIdx, unsigned int axis,
  ...
  for (ii = 0; ii <= res - 1; ii++) {
    ...
    switch (axis) {
    case 0:
      tmp = limnObjectVertexAdd(obj, partIdx, 0, AIR_FLOAT(-sin(th)),
                                AIR_FLOAT(cos(th)));
      break;
    case 1:
      tmp = limnObjectVertexAdd(obj, partIdx, AIR_FLOAT(sin(th)), 0, AIR_FLOAT(cos(th)));
      break;
    case 2:
    default:
      tmp = limnObjectVertexAdd(obj, partIdx, AIR_FLOAT(cos(th)), AIR_FLOAT(sin(th)), 0);
      break;
    }
    if (!ii) {
      vII0 = tmp;
...
limnObjectPolarSphereAdd(limnObject *obj, unsigned int lookIdx, unsigned int axis,
  ...
  for (pi = 1; pi <= phiRes - 1; pi++) {
    ...
    for (ti = 0; ti <= thetaRes - 1; ti++) {
      ...
      switch (axis) {
      case 0:
        limnObjectVertexAdd(obj, partIdx, AIR_FLOAT(cos(p)), AIR_FLOAT(-sin(t) * sin(p)),
                            AIR_FLOAT(cos(t) * sin(p)));
        break;
      case 1:
        limnObjectVertexAdd(obj, partIdx, AIR_FLOAT(sin(t) * sin(p)), AIR_FLOAT(cos(p)),
                            AIR_FLOAT(cos(t) * sin(p)));
        break;
      case 2:
      default:
        limnObjectVertexAdd(obj, partIdx, AIR_FLOAT(cos(t) * sin(p)),
                            AIR_FLOAT(sin(t) * sin(p)), AIR_FLOAT(cos(p)));
        break;
      }
    }
  }
...
limnObjectPolarSuperquadFancyAdd(limnObject *obj, unsigned int lookIdx,
  ...
  for (pi = 1; pi <= phiRes - 1; pi++) {
    ...
    for (ti = 0; ti <= thetaRes - 1; ti++) {
      ...
      limnObjectVertexAdd(obj, partIdx, AIR_FLOAT(x), AIR_FLOAT(y), AIR_FLOAT(z));
    }
  }
  switch (axis) {
  case 0:
...
limnObjectPolarSuperquadAdd(limnObject *obj, unsigned int lookIdx, unsigned int axis,
  ...
  return limnObjectPolarSuperquadFancyAdd(obj, lookIdx, axis, A, B, B, 0, thetaRes,
                                          phiRes);
}
