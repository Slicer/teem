...
plotLine(plotPS *pps, plotParm *pparm,
	 double width, double x0, double y0, double x1, double y1) {
...
void
plotLabel(plotPS *pps, plotParm *pparm,
...
plotAxes(plotPS *pps, plotParm *pparm, Nrrd *ndata) {
  ...
  if (strlen(pparm->axisHorzLabel) || strlen(pparm->axisVertLabel)) {
    ...
    if (strlen(pparm->axisHorzLabel)) {
      plotLabel(pps, pparm, pps->bbox[2], axY, pparm->axisHorzLabel);
    }
...
main(int argc, char *argv[]) {
  ...
  hestOptAdd(&hopt, "grth", "graph thickness", airTypeDouble, 1, -1, &(pparm.graphThick), "0.01",
	     "thickness of line for graph, or \"0\" for no graph line", &numGrth);
  hestOptAdd(&hopt, "grgr", "graph gray", airTypeDouble, 1, -1, &(pparm.graphGray), "0",
	     "grayscale to use for graph", &numGrgr);
  hestOptAdd(&hopt, "dtdi", "dot diameter", airTypeDouble, 1, -1, &(pparm.dotDiameter), "0.1",
	     "radius of dot drawn at data points, or \"0\" for no dots", &numDtdi);
  ...
  hestOptAdd(&hopt, "axth", "axis thickness", airTypeDouble, 1, 1, &(pparm.axisThick), "0.01",
  ...
  hestOptAdd(&hopt, "axhl", "horiz axis label", airTypeString, 1, 1, &(pparm.axisHorzLabel), "",
	     "label on horizontal axis");
  hestOptAdd(&hopt, "axvl", "vert axis label", airTypeString, 1, 1, &(pparm.axisVertLabel), "",
