...
_miteUserCheck(miteUser *muu) {
  ...
  for (T = 0; T < muu->ntxfNum; T++) {
    if (miteNtxfCheck(muu->ntxf[T])) {
      biffAddf(MITE, "%s: ntxf[%d] (%d of %d) can't be used as a txf", me, T, T + 1,
               muu->ntxfNum);
      airMopError(mop);
      return 1;
    }
  ...
  if (airStrlen(muu->normalStr)) {
    ...
    if (miteValGageKind == isp.kind) {
      biffAddf(MITE,
               "%s: normalStr \"%s\" refers to a miteVal "
               "(normal must be data-intrinsic)",
               me, muu->normalStr);
      airMopError(mop);
      return 1;
    }
    if (3 != isp.kind->table[isp.item].answerLength) {
      biffAddf(MITE, "%s: %s not a vector: can't be used as normal", me, muu->normalStr);
      return 1;
    }
    miteQueryAdd(queryScl, queryVec, queryTen, queryMite, &isp);
  }
  ...
  if (miteShadeSpecParse(shpec, muu->shadeStr)) {
    biffAddf(MITE, "%s: couldn't parse shading spec \"%s\"", me, muu->shadeStr);
    airMopError(mop);
    return 1;
  }
  ...
  if ((GAGE_QUERY_ITEM_TEST(queryMite, miteValNdotV)
    ...
    biffAddf(MITE,
             "%s: txf or shading requested a miteVal's use of the "
  ...
  if (GAGE_QUERY_NONZERO(queryScl) && !(muu->nsin)) {
    biffAddf(MITE, "%s: txf or shading require %s volume, but don't have one", me,
             gageKindScl->name);
    airMopError(mop);
    return 1;
  }
  ...
  if (muu->nsin) {
    if (gageVolumeCheck(muu->gctx0, muu->nsin, gageKindScl)) {
      biffMovef(MITE, GAGE, "%s: trouble with input %s volume", me, gageKindScl->name);
      airMopError(mop);
      return 1;
    }
  }
  if (muu->nvin) {
    if (gageVolumeCheck(muu->gctx0, muu->nvin, gageKindVec)) {
      biffMovef(MITE, GAGE, "%s: trouble with input %s volume", me, gageKindVec->name);
      airMopError(mop);
      return 1;
    }
  }
  if (muu->ntin) {
    if (gageVolumeCheck(muu->gctx0, muu->ntin, tenGageKind)) {
      biffMovef(MITE, GAGE, "%s: trouble with input %s volume", me, tenGageKind->name);
      airMopError(mop);
      return 1;
    }
  }
