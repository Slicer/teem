...
nrrdDeringInputSet(NrrdDeringContext *drc, const Nrrd *nin) {
  ...
  drc->sliceSize = (nin->axis[0].size * nin->axis[1].size * nrrdElementSize(nin));
  if (drc->verbose > 2) {
    fprintf(stderr, "%s: sliceSize = %u\n", me, AIR_UINT(drc->sliceSize));
  }
...
nrrdDeringClampPercSet(NrrdDeringContext *drc, double lo, double hi) {
  ...
  if (!(AIR_EXISTS(lo) && AIR_EXISTS(hi) && lo >= 0 && lo < CLAMP_PERC_MAX && hi >= 0
        && hi < CLAMP_PERC_MAX)) {
    biffAddf(NRRD, "%s: need finite lo and hi both in [0.0, %g), not %g, %g", me,
             CLAMP_PERC_MAX, lo, hi);
    return 1;
  }
...
nrrdDeringClampHistoBinsSet(NrrdDeringContext *drc, unsigned int bins) {
  ...
  if (!(bins >= CLAMP_HIST_BINS_MIN)) {
    biffAddf(NRRD, "%s: given bins %u not >= reasonable min %u", me, bins,
             CLAMP_HIST_BINS_MIN);
    return 1;
  }
...
deringBagNew(NrrdDeringContext *drc, double radMax) {
  ...
  airMopAdd(dbg->mop, dbg->nsliceOrig, (airMopper)nrrdNix /* not Nuke! */, airMopAlways);
  dbg->nslice = nrrdNew();
  airMopAdd(dbg->mop, dbg->nslice, (airMopper)nrrdNuke, airMopAlways);
  dbg->nsliceOut = nrrdNew();
  airMopAdd(dbg->mop, dbg->nsliceOut, (airMopper)nrrdNix /* not Nuke! */, airMopAlways);
  ...
  airMopAdd(dbg->mop, dbg->rsmc[0], (airMopper)nrrdResampleContextNix, airMopAlways);
  dbg->rsmc[1] = nrrdResampleContextNew();
  airMopAdd(dbg->mop, dbg->rsmc[1], (airMopper)nrrdResampleContextNix, airMopAlways);
  dbg->ringMag = 0.0;
...
deringPtxfAlloc(NrrdDeringContext *drc, deringBag *dbg) {
  ...
  for (ri = 0; ri < 2; ri++) {
    ...
    if (0 == ri) {
      ...
      if (!E) E |= nrrdResampleKernelSet(dbg->rsmc[0], 0, drc->rkernel, drc->rkparm);
      ...
      if (!E) E |= nrrdResampleKernelSet(dbg->rsmc[1], 1, drc->tkernel, drc->tkparm);
      if (drc->verticalSeam) {
    ...
    if (!E)
      E |= nrrdResampleNonExistentSet(dbg->rsmc[ri], nrrdResampleNonExistentRenormalize);
...
deringSliceGet(NrrdDeringContext *drc, deringBag *dbg, unsigned int zi) {
  ...
  /* slice setup */
  if (nrrdWrap_va(dbg->nsliceOrig, nonconstdata, drc->nin->type, 2,
                  drc->nin->axis[0].size, drc->nin->axis[1].size)
...
deringSliceSet(NrrdDeringContext *drc, deringBag *dbg, Nrrd *nout, unsigned int zi) {
  ...
  if (nrrdWrap_va(dbg->nsliceOut, AIR_VOIDP(drc->cdataOut + zi * (drc->sliceSize)),
                  nout->type, 2, nout->axis[0].size, nout->axis[1].size)
...
static void
deringXYtoRT(NrrdDeringContext *drc, deringBag *dbg, unsigned int xi, unsigned int yi,
             unsigned int *rrIdx, unsigned int *thIdx, double *rrFrc, double *thFrc) {
  ...
  for (yi = 0; yi < sy; yi++) {
    for (xi = 0; xi < sx; xi++) {
      ...
      if (drc->linearInterp) {
        ...
        bidxPlus = AIR_UINT(thIdx < drc->thetaNum - 1 ? bidx + dbg->radNum : rrIdx);
...
deringPtxfFilter(NrrdDeringContext *drc, deringBag *dbg, unsigned int zi) {
  ...
  if ((!zi ? 0 : nrrdResampleInputSet(dbg->rsmc[0], dbg->nptxf[ORIG]))
      ...
      || nrrdArithBinaryOp(dbg->nptxf[DIFF], nrrdBinaryOpSubtract, dbg->nptxf[ORIG],
                           dbg->nptxf[BLRR])) {
  ...
  if (!drc->verticalSeam) {
    ...
    pmax[2] = cmax[2] = 1;
    if (nrrdAxesSplit(dbg->nptxf[RSHP], dbg->nptxf[DIFF], 1, drc->thetaNum / 2, 2)
        ...
        || nrrdPad_nva(dbg->nptxf[RSHP], dbg->nptxf[CBLR], pmin, pmax, nrrdBoundaryPad,
                       0)
    ...
    if (DEBUG) {
      ...
      nrrdSave(fn, dbg->nptxf[CROP], NULL);
    }
  }
  if (DEBUG) {
    char fn[AIR_STRLEN_SMALL + 1];
    ...
    nrrdSave(fn, dbg->nptxf[BLRR], NULL);
    sprintf(fn, "diff-%02u.nrrd", dbg->zi);
    nrrdSave(fn, dbg->nptxf[DIFF], NULL);
    sprintf(fn, "ring-%02u.nrrd", dbg->zi);
    nrrdSave(fn, dbg->nptxf[RING], NULL);
  }
...
deringDo(NrrdDeringContext *drc, deringBag *dbg, Nrrd *nout, unsigned int zi) {
  ...
  if (deringSliceGet(drc, dbg, zi) /* */
      || deringPtxfDo(drc, dbg) || deringPtxfFilter(drc, dbg, zi)
      || deringRingMagMeasure(drc, dbg) || deringSubtract(drc, dbg)
...
deringCheck(NrrdDeringContext *drc) {
  ...
  if (drc->verticalSeam && !(0 == (drc->thetaNum % 2))) {
    biffAddf(NRRD, "%s: need even thetaNum (not %u) if wanting verticalSeam", me,
             drc->thetaNum);
    return 1;
  }
  return 0;
...
nrrdDeringExecute(NrrdDeringContext *drc, Nrrd *nout) {
  ...
  if (drc->clampPerc[0] > 0.0 || drc->clampPerc[1] > 0.0) {
    ...
    if (nrrdHisto(nhist, drc->nin, NULL, NULL, drc->clampHistoBins, nrrdTypeDouble)) {
    ...
    for (hi = 0; hi < drc->clampHistoBins; hi++) {
      ...
      if (sum >= drc->clampPerc[0] * total / 100.0) {
        drc->clamp[0] = AIR_AFFINE(0, hi, drc->clampHistoBins - 1, nhist->axis[0].min,
                                   nhist->axis[0].max);
        break;
      }
    }
    ...
    if (drc->verbose) {
      fprintf(stderr, "%s: [%g,%g]-%%ile clamping of [%g,%g] --> [%g,%g]\n", me,
              drc->clampPerc[0], drc->clampPerc[1], nhist->axis[0].min,
              nhist->axis[0].max, drc->clamp[0], drc->clamp[1]);
    }
  ...
  sz = (2 == drc->nin->dim ? 1 : AIR_UINT(drc->nin->axis[2].size));
