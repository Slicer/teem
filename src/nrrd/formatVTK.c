...
_nrrdFormatVTK_fitsInto(const Nrrd *nrrd, const NrrdEncoding *encoding, int useBiff) {
  ...
  if (!(nrrdTypeUChar == nrrd->type || nrrdTypeChar == nrrd->type
        || nrrdTypeUShort == nrrd->type || nrrdTypeShort == nrrd->type
        || nrrdTypeUInt == nrrd->type || nrrdTypeInt == nrrd->type
        || nrrdTypeFloat == nrrd->type || nrrdTypeDouble == nrrd->type)) {
    biffMaybeAddf(useBiff, NRRD,
                  "%s: type %s doesn't fit in VTK (as currently implemented)", me,
                  airEnumStr(nrrdType, nrrd->type));
    return AIR_FALSE;
  }
  if (!(3 == nrrd->dim || (4 == nrrd->dim && 3 == nrrd->axis[0].size)
        || (4 == nrrd->dim && 9 == nrrd->axis[0].size))) {
    biffMaybeAddf(useBiff, NRRD,
                  "%s: nrrd didn't look like a volume of "
...
_nrrdFormatVTK_read(FILE *file, Nrrd *nrrd, NrrdIoState *nio) {
  ...
  if (!_nrrdFormatVTK_contentStartsLike(nio)) {
    biffAddf(NRRD, "%s: this doesn't look like a %s file", me, nrrdFormatVTK->name);
    return 1;
  }
...
#define GETLINE(what)                                                                   \
  ...
  if (!strcmp("ASCII", nio->line)) {
    ...
    biffAddf(NRRD, "%s: encoding \"%s\" wasn't \"ASCII\" or \"BINARY\"", me, nio->line);
    return 1;
  }
  GETLINE(DATASET);
  airToUpper(nio->line);
  ...
  if (!strstr(nio->line, "DIMENSIONS")
    ...
    biffAddf(NRRD, "%s: couldn't parse DIMENSIONS line (\"%s\")", me, nio->line);
    return 1;
  }
  GETLINE(next);
  airToUpper(nio->line);
  while (!strstr(nio->line, "POINT_DATA")) {
    if (strstr(nio->line, "ORIGIN")) {
      if (3 != sscanf(nio->line, "ORIGIN %lf %lf %lf", &xm, &ym, &zm)) {
        biffAddf(NRRD, "%s: couldn't parse ORIGIN line (\"%s\")", me, nio->line);
        return 1;
      }
    } else if (strstr(nio->line, "SPACING")) {
      if (3 != sscanf(nio->line, "SPACING %lf %lf %lf", &xs, &ys, &zs)) {
        biffAddf(NRRD, "%s: couldn't parse SPACING line (\"%s\")", me, nio->line);
        return 1;
      }
    } else if (strstr(nio->line, "ASPECT_RATIO")) {
      if (3 != sscanf(nio->line, "ASPECT_RATIO %lf %lf %lf", &xs, &ys, &zs)) {
        biffAddf(NRRD, "%s: couldn't parse ASPECT_RATIO line (\"%s\")", me, nio->line);
        return 1;
      }
    }
    GETLINE(next);
  ...
  if (N != sx * sy * sz) {
    biffAddf(NRRD, "%s: product of sizes (%u*%u*%u == %u) != # elements (%d)", me, sx,
             sy, sz, sx * sy * sz, N);
    return 1;
  }
  ...
  if (!strcmp(three[2], "bit")) {
    if (nrrdEncodingAscii == nio->encoding) {
      fprintf(stderr,
              "%s: WARNING: \"bit\"-type data will be read in as "
              "unsigned char\n",
              me);
...
_nrrdFormatVTK_write(FILE *file, const Nrrd *_nrrd, NrrdIoState *nio) {
  ...
  if (!(3 == nrrd->dim
        || (4 == nrrd->dim && (3 == nrrd->axis[0].size || 9 == nrrd->axis[0].size)))) {
