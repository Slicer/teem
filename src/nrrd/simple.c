...
nrrdSanity(void) {
  ...
  */
  ...
  if (!(nrrdTypeDefault == nrrdDefaultResampleType
    ...
    biffAddf(NRRD, "%s: nrrdDefaultResampleType (%d) not in valid range [%d,%d]", me,
             nrrdDefaultResampleType, nrrdTypeUnknown, nrrdTypeLast - 1);
    return 0;
  }
  if (airEnumValCheck(nrrdBoundary, nrrdDefaultResampleBoundary)) {
    biffAddf(NRRD,
             "%s: nrrdDefaultResampleBoundary (%d) "
             "not in valid range [%d,%d]",
             me, nrrdDefaultResampleBoundary, nrrdBoundaryUnknown + 1,
             nrrdBoundaryLast - 1);
    return 0;
  }
  if (airEnumValCheck(nrrdType, nrrdStateMeasureType)) {
    biffAddf(NRRD, "%s: nrrdStateMeasureType (%d) not in valid range [%d,%d]", me,
             nrrdStateMeasureType, nrrdTypeUnknown + 1, nrrdTypeLast - 1);
    return 0;
  }
  if (airEnumValCheck(nrrdType, nrrdStateMeasureHistoType)) {
    biffAddf(NRRD,
             ...
             "valid range [%d,%d]",
             me, nrrdStateMeasureType, nrrdTypeUnknown + 1, nrrdTypeLast - 1);
    return 0;
  }
  /* ---- END non-NrrdIO */
