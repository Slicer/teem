...
nrrdSliceSelect(Nrrd *noutAbove, Nrrd *noutBelow, const Nrrd *nin, unsigned int saxi,
  ...
  NrrdRange *rng;
  double *line;
  size_t II, LL, numAbove, numBelow, stride, size[2 /* 0=above 1=below */][NRRD_DIM_MAX];
  ...
  if (!(saxi < nin->dim)) {
    biffAddf(NRRD, "%s: can't select axis %u of a %u-D input nrrd", me, saxi, nin->dim);
    return 1;
  }
  ...
  if (!(nrrdTypeBlock != _nline->type && 1 == _nline->dim)) {
    biffAddf(NRRD, "%s: nline must be 1-D array of scalars (not %u-D of %s)", me,
             _nline->dim, airEnumStr(nrrdType, _nline->type));
    return 1;
  }
  ...
  if (1 == nin->dim) {
    biffAddf(NRRD,
             "%s: sorry, current slice-based implementation "
             "requires input dimension > 1",
             me);
    return 1;
  }
  ...
  if (noutAbove) {
    if (!numAbove) {
      biffAddf(NRRD,
               "%s: want slices for val >= thresh %g, "
               "but highest value is %g < %g\n",
               me, thresh, rng->max, thresh);
      airMopError(mop);
      return 1;
    }
    noutPrep[0] = nrrdNew();
  ...
  for (oi = 0; oi <= 1; oi++) {
    if (nout[oi]) /* noutAbove or noutBelow */ {
      if (nrrdMaybeAlloc_nva(noutPrep[oi], nin->type, nin->dim, size[oi])) {
        biffAddf(NRRD, "%s: trouble allocating output for slices %s", me,
                 !oi ? "above" : "below");
        airMopError(mop);
        return 1;
      }
    ...
    for (oi = 0; oi <= 1; oi++) {
      if (test[oi] && nout[oi] /* noutAbove or noutBelow */) {
        ...
        if (nrrdSlice(nslice, nin, saxi, II)) {
          biffAddf(NRRD, "%s: trouble slicing (%s) at %s", me, !oi ? "above" : "below",
          ...
          || nrrdBasicInfoCopy(nout[oi], nin,
                               NRRD_BASIC_INFO_DATA_BIT /* */
                                 ...
                                 | (nrrdStateKeyValuePairsPropagate
                                      ? 0
                                      : NRRD_BASIC_INFO_KEYVALUEPAIRS_BIT))) {
        biffAddf(NRRD, "%s: trouble with %s output", me, !oi ? "above" : "below");
        return 1;
      }
    }
  }
...
nrrdSample_nva(void *val, const Nrrd *nrrd, const size_t *coord) {
  ...
  for (ai = 0; ai < nrrd->dim; ai++) {
    if (!(coord[ai] < size[ai])) {
      biffAddf(NRRD, "%s: coordinate %s on axis %d out of bounds (0 to %s)", me,
               airSprintSize_t(stmp[0], coord[ai]), ai,
               airSprintSize_t(stmp[1], size[ai] - 1));
      return 1;
    }
  }
...
nrrdCropAuto(Nrrd *nout, const Nrrd *nin, size_t _min[NRRD_DIM_MAX],
  ...
  if (airEnumValCheck(nrrdMeasure, measr)) {
    biffAddf(NRRD, "%s: invalid %s measr %d", me, nrrdMeasure->name, measr);
    return 1;
  }
  ...
  for (axi = 0; axi < keepNum; axi++) {
    if (!(keep[axi] < nin->dim)) {
      biffAddf(NRRD, "%s: keep[%u] %u out of range [0,%u]", me, axi, keep[axi],
               nin->dim - 1);
      return 1;
    }
    if (!cropdo[keep[axi]]) {
