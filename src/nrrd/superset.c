...
nrrdSplice(Nrrd *nout, const Nrrd *nin, const Nrrd *nslice, unsigned int axis,
  ...
  if (!(axis < nin->dim)) {
    biffAddf(NRRD, "%s: slice axis %d out of bounds (0 to %d)", me, axis, nin->dim - 1);
    return 1;
  }
  ...
  if (!(nin->dim - 1 == nslice->dim)) {
    biffAddf(NRRD,
             "%s: dim of slice (%d) not one less than "
             "dim of input (%d)",
             me, nslice->dim, nin->dim);
    return 1;
  }
  if (!(nin->type == nslice->type)) {
    biffAddf(NRRD, "%s: type of slice (%s) != type of input (%s)", me,
             airEnumStr(nrrdType, nslice->type), airEnumStr(nrrdType, nin->type));
    return 1;
  }
  ...
  for (ai = 0; ai < nslice->dim; ai++) {
    ...
    if (indexOffset) {
      ...
      foundExitCondition = 1;
    if (foundExitCondition) {
      biffAddf(NRRD, "%s: input ax %u size (%s) != slices ax %u size (%s)", me,
               ai + indexOffset,
...
nrrdInset(Nrrd *nout, const Nrrd *nin, const Nrrd *nsub, const size_t *min) {
  ...
  if (!(nin->dim == nsub->dim)) {
    biffAddf(NRRD, "%s: input's dim (%d) != subvolume's dim (%d)", me, nin->dim,
             nsub->dim);
    return 1;
  }
  if (!(nin->type == nsub->type)) {
    biffAddf(NRRD, "%s: input's type (%s) != subvolume's type (%s)", me,
             airEnumStr(nrrdType, nin->type), airEnumStr(nrrdType, nsub->type));
    return 1;
  }
  if (nrrdTypeBlock == nin->type) {
  ...
  for (ai = 0; ai < nin->dim; ai++) {
    if (!(min[ai] + nsub->axis[ai].size - 1 <= nin->axis[ai].size - 1)) {
      biffAddf(NRRD,
               "%s: axis %d range of inset indices [%s,%s] not within "
               "input indices [0,%s]",
  ...
  strcpy(buff1, "[");
  for (ai = 0; ai < nin->dim; ai++) {
    sprintf(buff2, "%s%s", (ai ? "," : ""), airSprintSize_t(stmp[0], min[ai]));
    strcat(buff1, buff2);
  }
  strcat(buff1, "]");
...
nrrdPad_va(Nrrd *nout, const Nrrd *nin, const ptrdiff_t *min, const ptrdiff_t *max,
  ...
  int outside; /* whether current sample in output has any coordinates
                  ...
                  not per-axis) */
  unsigned int ai;
  ptrdiff_t cIn[NRRD_DIM_MAX];    /* coords for line start, in input */
  size_t typeSize, idxIn, idxOut, /* linear indices for input and output */
  ...
  default:
    biffAddf(NRRD, "%s: sorry boundary %s (%d) unimplemented\n", me,
             airEnumStr(nrrdBoundary, boundary), boundary);
    return 1;
  }
  /*
  ...
  */
  ...
  for (idxOut = 0; idxOut < numOut; idxOut++) {
    ...
    if (!outside) {
      /* the cIn coords are within the input nrrd: do memcpy() of whole
         ...
         the scanline */
      memcpy(dataOut + idxOut * typeSize, dataIn + idxIn * typeSize, szIn[0] * typeSize);
  ...
  strcpy(buff1, "");
  for (ai = 0; ai < nin->dim; ai++) {
    sprintf(buff2, "%s[%s,%s]", (ai ? "x" : ""), airSprintPtrdiff_t(stmp[0], min[ai]),
            airSprintPtrdiff_t(stmp[1], max[ai]));
    strcat(buff1, buff2);
  }
  if (nrrdBoundaryPad == boundary) {
    sprintf(buff2, "%s(%g)", airEnumStr(nrrdBoundary, nrrdBoundaryPad), padValue);
  } else {
