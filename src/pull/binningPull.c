...
_pullBinNeighborSet(pullContext *pctx, pullBin *bin) {
  ...
  unsigned int xi, yi, zi, si, xx, yy, zz, ss, xmax, ymax, zmax, smax;
  pullBin *nei[3 * 3 * 3 * 3];
...
_pullBinLocate(pullContext *pctx, double *posWorld) {
  ...
  if (!ELL_4V_EXISTS(posWorld)) {
    biffAddf(PULL, "%s: non-existent position (%g,%g,%g,%g)", me, posWorld[0],
  ...
  if (pctx->flag.binSingle) {
    ...
    binIdx
      = (eidx[0]
         + pctx->binsEdge[0]
             * (eidx[1] + pctx->binsEdge[1] * (eidx[2] + pctx->binsEdge[2] * eidx[3])));
  }
...
_pullBinPointAdd(pullContext *pctx, pullBin *bin, pullPoint *point) {
  ...
  if (!(bin->pointArr)) {
    ...
    bin->pointArr = airArrayNew(pppu.v, &(bin->pointNum), sizeof(pullPoint *),
                                _PULL_BIN_INCR);
    if (!(bin->pointArr)) {
...
pullBinsPointAdd(pullContext *pctx, pullPoint *point, pullBin **binP) {
  ...
  if (!(bin = _pullBinLocate(pctx, point->pos))) {
    biffAddf(PULL, "%s: can't locate point %p %u", me, AIR_VOIDP(point), point->idtag);
    return 1;
  }
  if (binP) {
    *binP = bin;
  ...
  if (_pullBinPointAdd(pctx, bin, point)) {
    biffAddf(PULL, "%s: trouble adding point %p %u", me, AIR_VOIDP(point), point->idtag);
    return 1;
  }
  return 0;
}
...
pullBinsPointMaybeAdd(pullContext *pctx, pullPoint *point,
  ...
  if (pctx->flag.restrictiveAddToBins) {
    ...
    if (okay) {
      if (_pullBinPointAdd(pctx, bin, point)) {
        biffAddf(PULL, "%s: trouble adding point %p %u", me, AIR_VOIDP(point),
                 point->idtag);
        return 1;
      }
      *added = AIR_TRUE;
...
_pullBinSetup(pullContext *pctx) {
  ...
  if (pctx->verbose) {
    printf("%s: radiusSpace = %g -(%g)-> maxDistSpace = %g\n", me,
           pctx->sysParm.radiusSpace, pctx->sysParm.binWidthSpace, pctx->maxDistSpace);
  ...
  if (pctx->flag.binSingle) {
    ...
    */
    if (pctx->verbose) {
      printf("%s: binsEdge=(%u,%u,%u,%u)\n", me, pctx->binsEdge[0], pctx->binsEdge[1],
  ...
  if (pctx->binNum > PULL_BIN_MAXNUM) {
    biffAddf(PULL,
             "%s: sorry, #bins %u > PULL_BIN_MAXNUM %u. Try "
             "increasing pctx->sysParm.binWidthSpace (%g)",
             me, pctx->binNum, PULL_BIN_MAXNUM, pctx->sysParm.binWidthSpace);
    return 1;
  }
  if (pctx->verbose) {
...
_pullIterFinishDescent(pullContext *pctx) {
  ...
  if (pointNum != pctx->tmpPointNum) {
    if (pctx->verbose) {
      printf("!%s: changing total point # %u --> %u\n", me, pctx->tmpPointNum, pointNum);
    }
    airFree(pctx->tmpPointPerm);
    airFree(pctx->tmpPointPtr);
    pctx->tmpPointPtr = AIR_CAST(pullPoint **, calloc(pointNum, sizeof(pullPoint *)));
  ...
  airShuffle_r(pctx->task[0]->rng, pctx->tmpPointPerm, pointNum,
               pctx->flag.permuteOnRebin);
