...
outputInit(seekContext *sctx, baggage *bag, limnPolyData *lpld) {
  ...
  if (seekTypeIsocontour == sctx->type
    ...
    valI = AIR_UINT(airIndex(sctx->range->min, sctx->isovalue, sctx->range->max,
                             AIR_UINT(sctx->spanSize)));
  ...
  bag->xyzwArr = airArrayNew((appu.f = &(lpld->xyzw), appu.v), &(lpld->xyzwNum),
                             4 * sizeof(float), sctx->pldArrIncr);
  if (sctx->normalsFind) {
    bag->normArr = airArrayNew((appu.f = &(lpld->norm), appu.v), &(lpld->normNum),
                               3 * sizeof(float), sctx->pldArrIncr);
  } else {
    bag->normArr = NULL;
  }
  bag->indxArr = airArrayNew((appu.ui = &(lpld->indx), appu.v), &(lpld->indxNum),
  ...
  if (E) {
    biffAddf(SEEK, "%s: couldn't pre-allocate contour geometry (%p %p %p)", me,
             bag->xyzwArr->data, (sctx->normalsFind ? bag->normArr->data : NULL),
             bag->indxArr->data);
    return 1;
  }
...
static double
sclGet(seekContext *sctx, baggage *bag, unsigned int xi, unsigned int yi,
       unsigned int zi) {
...
void
_seekIdxProbe(seekContext *sctx, baggage *bag, double xi, double yi, double zi) {
...
evecFlipProbe(seekContext *sctx, baggage *bag, signed char *flip, /* OUTPUT HERE */
  ...
  if (!(xi + dx < sx && yi + dy < sy && bag->zi + ziOff + dz < sz)) {
  ...
  ELL_3V_COPY(evecA, sctx->evec + 3 * (bag->esIdx + 3 * (ziOff + 2 * (xi + sx * yi))));
  ELL_3V_COPY(evecB,
              sctx->evec
...
#define SETNEXT(uu)                                                                     \
  ...
  while (u + du < 1.0) {
    ...
    while (dot < wantDot) {
      ...
      if (du < minDu) {
        fprintf(stderr,
                "%s: evector wild @ u=%g: du=%g < minDu=%g; "
                "dot=%g; mode = %g; "
                "(xi,yi,zi)=(%u,%u,%u+%u); (dx,dy,dz)=(%u,%u,%u) ",
                me, u, du, minDu, dot, mode, xi, yi, bag->zi, ziOff, dx, dy, dz);
        *flip = 0;
        return 0;
      }
      SETNEXT(u + du);
    ...
    for (xi = 0; xi < sx; xi++) {
      ...
      if (sctx->treated[si] & 0x01 || (xi != 0 && sctx->treated[xi - 1 + sx * yi] & 0x01)
        ...
        if (evecFlipProbe(sctx, bag, &flipA, xi, yi, 0, 0, 0, 1)) {
          biffAddf(SEEK, "%s: problem at (xi,yi,zi) = (%u,%u,%u)", me, xi, yi, bag->zi);
          return 1;
        }
    ...
    for (xi = 0; xi < sx; xi++) {
      ...
      if (!bag->zi) {
        ...
        if (sctx->strengthUse) {
          ...
          sctx->strengthSeenMax = AIR_MAX(sctx->strengthSeenMax, sctx->stng[0 + 2 * si]);
        }
        switch (sctx->type) {
        case seekTypeIsocontour:
          sctx->sclv[0 + 4 * spi] = (sclGet(sctx, bag, xi, yi, 0) - sctx->isovalue);
          sctx->sclv[1 + 4 * spi] = (sclGet(sctx, bag, xi, yi, 0) - sctx->isovalue);
          sctx->sclv[2 + 4 * spi] = (sclGet(sctx, bag, xi, yi, 1) - sctx->isovalue);
          break;
        case seekTypeRidgeSurface:
        case seekTypeValleySurface:
        case seekTypeMaximalSurface:
      ...
      if (sctx->strengthUse) {
        ...
        sctx->strengthSeenMax = AIR_MAX(sctx->strengthSeenMax, sctx->stng[1 + 2 * si]);
...
static void
vvalIsoSet(seekContext *sctx, baggage *bag, double vval[8], unsigned int xi,
           unsigned int yi) {
  ...
  if (sctx->strengthUse) {
    ...
    ssum = wsum = 0;
#define ACCUM(vi)                                                                       \
  w = AIR_ABS(1.0 / vval[vi]);                                                          \
  ssum += w * s;                                                                        \
  wsum += w
    s = sctx->stng[0 + 2 * (0 + 0 * sx + si)];
    ACCUM(0);
    s = sctx->stng[0 + 2 * (1 + 0 * sx + si)];
    ACCUM(1);
    s = sctx->stng[0 + 2 * (0 + 1 * sx + si)];
    ACCUM(2);
    s = sctx->stng[0 + 2 * (1 + 1 * sx + si)];
    ACCUM(3);
    s = sctx->stng[1 + 2 * (0 + 0 * sx + si)];
    ACCUM(4);
    s = sctx->stng[1 + 2 * (1 + 0 * sx + si)];
    ACCUM(5);
    s = sctx->stng[1 + 2 * (0 + 1 * sx + si)];
    ACCUM(6);
    s = sctx->stng[1 + 2 * (1 + 1 * sx + si)];
    ACCUM(7);
#undef ACCUM
...
vvalSurfSet(seekContext *sctx, baggage *bag, double vval[8], unsigned int xi,
  ...
  flipProd = 1;
  if (sctx->type != seekTypeRidgeSurfaceOP && sctx->type != seekTypeValleySurfaceOP) {
    for (ei = 0; ei < 12; ei++) {
  ...
  if ((sctx->strengthUse && maxStrength < sctx->strength) || !flipProd) {
    ...
    if (sctx->type == seekTypeRidgeSurfaceOP || sctx->type == seekTypeValleySurfaceOP) {
      ...
      for (vi = 0; vi < 8; ++vi) {
        if (ELL_3V_DOT(evec[vi], outerevecs) < 0) ELL_3V_SCALE(evec[vi], -1.0, evec[vi]);
      }
    } else {
...
triangulate(seekContext *sctx, baggage *bag, limnPolyData *lpld) {
  ...
  int e2v[12][2] = {
    /* maps edge index to corner vertex indices */
    {0, 1}, /*  0 */
    {0, 2}, /*  1 */
    {1, 3}, /*  2 */
    {2, 3}, /*  3 */
  ...
  for (yi = 0; yi < sy - 1; yi++) {
    ...
    for (xi = 0; xi < sx - 1; xi++) {
      ...
      if (seekTypeIsocontour == sctx->type && sctx->normalsFind && !sctx->normAns) {
      ...
      for (ei = 0; ei < 12; ei++) {
        if ((ecase & (1 << ei)) && -1 == sctx->vidx[bag->evti[ei] + 5 * si]) {
          int ovi;
          ...
          ovi = sctx->vidx[bag->evti[ei] + 5 * si] = airArrayLenIncr(bag->xyzwArr, 1);
          ELL_4V_SET_TT(lpld->xyzw + 4 * ovi, float, tvertA[0], tvertA[1], tvertA[2],
                        1.0);
          /*
...
surfaceExtract(seekContext *sctx, limnPolyData *lpld) {
  ...
  for (zi = 0; zi < sz - 1; zi++) {
    ...
    if (sctx->type == seekTypeRidgeSurfaceT || sctx->type == seekTypeValleySurfaceT) {
      if (_seekShuffleProbeT(sctx, bag) || _seekTriangulateT(sctx, bag, lpld))
        trouble = 1;
    } else {
      if (shuffleProbe(sctx, bag) || triangulate(sctx, bag, lpld)) trouble = 1;
    }
    if (trouble) {
