...
updateAnswerPointers(seekContext *sctx) {
  ...
  if (sctx->flag[flagItemValue] || sctx->flag[flagItemStrength]
      || sctx->flag[flagItemNormal] || sctx->flag[flagItemGradient]
      || sctx->flag[flagItemEigensystem] || sctx->flag[flagItemHess]
      || sctx->flag[flagNinEtAl] || sctx->flag[flagNormalsFind]
      || sctx->flag[flagStrengthUse] || sctx->flag[flagType]) {
    ...
    if (sctx->strengthUse) {
      ...
      sctx->stngAns = (gageAnswerPointer(sctx->gctx, sctx->pvl, sctx->stngItem));
    } else {
      sctx->stngAns = NULL;
    }
    ...
    case seekTypeIsocontour:
      if (!(sctx->ninscl || -1 != sctx->sclvItem)) {
        biffAddf(SEEK, "%s: need either scalar volume or value item set for %s", me,
      ...
      if (sctx->normalsFind) {
        ...
        if (!(sctx->ninscl || -1 != sctx->normItem)) {
          biffAddf(SEEK,
                   "%s: need either scalar volume or "
      ...
      if (sctx->ninscl) {
        ...
        sctx->sclvAns = gageAnswerPointer(sctx->gctx, sctx->pvl, sctx->sclvItem);
        sctx->normAns = (sctx->normalsFind
                           ? gageAnswerPointer(sctx->gctx, sctx->pvl, sctx->normItem)
                           : NULL);
      }
      if (sctx->flag[flagItemGradient] || sctx->flag[flagItemEigensystem]
          || sctx->flag[flagItemHess]) {
        biffAddf(SEEK, "%s: can't set gradient, Hessian, or eigensystem for %s", me,
                 airEnumStr(seekType, seekTypeIsocontour));
        return 1;
      }
      sctx->gradAns = NULL;
    ...
    case seekTypeValleySurfaceT:
      if (!sctx->pvl) {
        biffAddf(SEEK, "%s: can't find %s without a gage context", me,
                 airEnumStr(seekType, sctx->type));
        return 1;
      }
      if (!(-1 != sctx->gradItem && -1 != sctx->evalItem && -1 != sctx->evecItem)) {
      ...
      if (sctx->normalsFind) {
        /* NOTE simplifying assumption described in seek.h */
        if (-1 == sctx->normItem) {
          biffAddf(SEEK, "%s: need normal item set for normals for %s", me,
                   airEnumStr(seekType, sctx->type));
          return 1;
        }
        sctx->normAns = (gageAnswerPointer(sctx->gctx, sctx->pvl, sctx->normItem));
      } else {
        sctx->normAns = NULL;
      }
      sctx->sclvAns = NULL;
      ...
      if (sctx->type == seekTypeRidgeSurfaceT || sctx->type == seekTypeValleySurfaceT)
        sctx->hessAns = gageAnswerPointer(sctx->gctx, sctx->pvl, sctx->hessItem);
      else
        sctx->hessAns = NULL;
      break;
    default:
...
updateSxSySz(seekContext *sctx) {
  ...
  if (sctx->flag[flagSamples] || sctx->flag[flagNinEtAl]) {
    if (0 == sctx->samples[0] || 0 == sctx->samples[1] || 0 == sctx->samples[2]) {
      ELL_3V_COPY(sizeOut, sizeIn);
    } else {
      if (!sctx->pvl) {
        biffAddf(SEEK,
                 "%s: can't specify # samples (%u,%u,%u) independent of "
                 "volume dimensions (%u,%u,%u) without a gage context",
                 me, AIR_UINT(sctx->samples[0]), AIR_UINT(sctx->samples[1]),
                 AIR_UINT(sctx->samples[2]), AIR_UINT(sizeIn[0]), AIR_UINT(sizeIn[1]),
                 AIR_UINT(sizeIn[2]));
        return 1;
      }
    ...
    for (axi = 0; axi < 3; axi++) {
      min = (nrrdCenterCell == sctx->shape->center ? -0.5 : 0.0);
      max = (nrrdCenterCell == sctx->shape->center ? sizeIn[axi] - 0.5
                                                   : sizeIn[axi] - 1.0);
      off[axi] = NRRD_POS(sctx->shape->center, min, max, sizeOut[axi], 0);
      scl[axi] = (NRRD_POS(sctx->shape->center, min, max, sizeOut[axi], 1) - off[axi]);
    }
...
updateReverse(seekContext *sctx) {
  ...
  if (sctx->verbose > 5) {
    fprintf(stderr, "%s: --------------------\n", me);
    fprintf(stderr, "%s: flagNinEtAl = %d\n", me, sctx->flag[flagNinEtAl]);
    fprintf(stderr, "%s: flagLowerInside = %d\n", me, sctx->flag[flagLowerInside]);
  }

  if (sctx->flag[flagNinEtAl] || sctx->flag[flagLowerInside]) {
    double mat[9];
    int reverse;
...
updateSlabCacheAlloc(seekContext *sctx) {
  ...
  if (sctx->flag[flagType] || sctx->flag[flagStrengthUse] /* kind of sloppy/overkill */
    ...
    if (seekTypeRidgeSurface == sctx->type || seekTypeValleySurface == sctx->type
        || seekTypeMaximalSurface == sctx->type || seekTypeMinimalSurface == sctx->type
        || seekTypeRidgeSurfaceOP == sctx->type || seekTypeRidgeSurfaceT == sctx->type
...
updateSclDerived(seekContext *sctx) {
  ...
  if (sctx->flag[flagType] || sctx->flag[flagNinEtAl]) {
    if (!(seekTypeIsocontour == sctx->type && sctx->pvl)) {
      nrrdEmpty(sctx->nsclDerived);
    } else {
      if (nrrdMaybeAlloc_va(sctx->nsclDerived, nrrdTypeDouble, 3, sctx->sx, sctx->sy,
                            sctx->sz)) {
        biffMovef(SEEK, NRRD, "%s: couldn't allocated derived scalar volume", me);
        return 1;
      }
      ...
      for (zi = 0; zi < sctx->sz; zi++) {
        ...
        for (yi = 0; yi < sctx->sy; yi++) {
          for (xi = 0; xi < sctx->sx; xi++) {
            ...
            val = sctx->sclvAns[0];
            if (!AIR_EXISTS(val)) {
              biffAddf(SEEK, "%s: probed scalar[%u,%u,%u] %g doesn't exist", me, xi, yi,
                       zi, val);
              return 1;
            }
...
updateSpanSpaceHist(seekContext *sctx) {
  ...
  if (sctx->verbose > 5) {
    fprintf(stderr, "%s: --------------------\n", me);
    fprintf(stderr, "%s: flagType = %d\n", me, sctx->flag[flagType]);
    fprintf(stderr, "%s: flagSclDerived = %d\n", me, sctx->flag[flagSclDerived]);
    fprintf(stderr, "%s: flagNinEtAl = %d\n", me, sctx->flag[flagNinEtAl]);
  }

  if (sctx->flag[flagType] || sctx->flag[flagSclDerived] || sctx->flag[flagNinEtAl]) {
    if (seekTypeIsocontour != sctx->type) {
...
updateResult(seekContext *sctx) {
  ...
  if (sctx->verbose > 5) {
    fprintf(stderr, "%s: --------------------\n", me);
    fprintf(stderr, "%s: flagIsovalue = %d\n", me, sctx->flag[flagIsovalue]);
    fprintf(stderr, "%s: flagAnswerPointers = %d\n", me, sctx->flag[flagAnswerPointers]);
    fprintf(stderr, "%s: flagType = %d\n", me, sctx->flag[flagType]);
    fprintf(stderr, "%s: flagSlabCacheAlloc = %d\n", me, sctx->flag[flagSlabCacheAlloc]);
    fprintf(stderr, "%s: flagSpanSpaceHist = %d\n", me, sctx->flag[flagSpanSpaceHist]);
    fprintf(stderr, "%s: flagNinEtAl = %d\n", me, sctx->flag[flagNinEtAl]);
    fprintf(stderr, "%s: flagReverse = %d\n", me, sctx->flag[flagReverse]);
    fprintf(stderr, "%s: flagTxfNormal = %d\n", me, sctx->flag[flagTxfNormal]);
  }

  if (seekTypeIsocontour != sctx->type && sctx->flag[flagIsovalue]) {
    biffAddf(SEEK, "%s: can't set isovalue for %s (only %s)", me,
             airEnumStr(seekType, sctx->type), airEnumStr(seekType, seekTypeIsocontour));
    return 1;
  }

  if (sctx->strengthUse && !sctx->stngAns) {
    biffAddf(SEEK, "%s: can't use feature strength without a strength item", me);
    return 1;
  }

  /* this seems to be a very pointless exercise */
  if (sctx->flag[flagIsovalue] || sctx->flag[flagEvalDiffThresh]
      || sctx->flag[flagAnswerPointers] || sctx->flag[flagStrengthUse]
      || sctx->flag[flagStrength] || sctx->flag[flagType]
      || sctx->flag[flagSlabCacheAlloc] || sctx->flag[flagSpanSpaceHist]
      || sctx->flag[flagNinEtAl] || sctx->flag[flagReverse]
