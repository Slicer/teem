...
_tenGaussian(double *retP, double m, double t, double s) {
  ...
  *retP = exp(earg) / den;
  if (!AIR_EXISTS(*retP)) {
    ...
    biffAddf(TEN, "%s: failed with ret = exp(%g)/%g = %g/%g = %g", me, earg, den,
             exp(earg), den, *retP);
    *retP = AIR_NAN;
    return 1;
  }
...
_tenRicianTrue(double *retP,
  ...
  *retP = exp(earg) * airBesselI0(barg) * moss;
  ...
  if (!AIR_EXISTS(*retP)) {
    biffAddf(TEN, "%s: m=%g, t=%g, s=%g", me, m, t, s);
    biffAddf(TEN, "%s: mos=%g, moss=%g, tos=%g, ss=%g", me, mos, moss, tos, ss);
    biffAddf(TEN, "%s: mos2=%g, tos2=%g, earg=%g, barg=%g", me, mos2, tos2, earg, barg);
    biffAddf(TEN, "%s: failed: ret=exp(%g)*bessi0(%g)*%g = %g * %g * %g = %g", me, earg,
             barg, moss, exp(earg), airBesselI0(barg), moss, *retP);
    *retP = AIR_NAN;
    return 1;
  }
  return 0;
...
_tenRicianSafe(double *retP, double m, double t, double s) {
  ...
  if (E) {
    biffAddf(TEN, "%s: failed with m=%g, t=%g, s=%g -> diff=%g", me, m, t, s, diff);
    *retP = AIR_NAN;
    return 1;
  }
  return 0;
...
_tenRician(double *retP,
  ...
  if (E) {
    biffAddf(TEN, "%s: failed with m=%g, t=%g, s=%g -> tos=%g", me, m, t, s, tos);
    *retP = AIR_NAN;
    return 1;
  }
  return 0;
...
_tenEstimateOutputInit(tenEstimateContext *tec) {
  ...
  TEN_T_SET(tec->ten, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN);
  tec->conf = AIR_NAN;
  tec->mdwi = AIR_NAN;
  tec->time = AIR_NAN;
...
tenEstimateContextNew() {
  ...
  if (tec) {
    ...
    tec->skipListArr = airArrayNew(appu.v, NULL, 2 * sizeof(unsigned int), 128);
...
void
tenEstimateVerboseSet(tenEstimateContext *tec, int verbose) {
  if (tec) {
    tec->verbose = verbose;
  }
  return;
...
tenEstimateMethodSet(tenEstimateContext *tec, int estimateMethod) {
  ...
  if (airEnumValCheck(tenEstimate1Method, estimateMethod)) {
    biffAddf(TEN, "%s: estimateMethod %d not a valid %s", me, estimateMethod,
...
tenEstimateValueMinSet(tenEstimateContext *tec, double valueMin) {
  ...
  if (!(AIR_EXISTS(valueMin) && valueMin > 0.0)) {
    biffAddf(TEN, "%s: given valueMin (%g) not existent and > 0.0", me, valueMin);
    return 1;
  }
...
tenEstimateThresholdFind(double *threshP, unsigned char *isB0, Nrrd *nin4d) {
  ...
  Nrrd **ndwi;
  airArray *mop;
  unsigned int slIdx, slNum, dwiAx, dwiNum, rangeAxisNum, rangeAxisIdx[NRRD_DIM_MAX];
  int dwiIdx;
...
tenEstimateThresholdSet(tenEstimateContext *tec, double thresh, double soft) {
  ...
  if (!(AIR_EXISTS(thresh) && AIR_EXISTS(soft))) {
    biffAddf(TEN, "%s: not both threshold (%g) and softness (%g) exist", me, thresh,
             soft);
    return 1;
  }
...
_tenEstimateCheck(tenEstimateContext *tec) {
  ...
  if (!(AIR_EXISTS(tec->valueMin) && tec->valueMin > 0.0)) {
    biffAddf(TEN, "%s: need a positive valueMin set (not %g)", me, tec->valueMin);
    return 1;
  }
  if (!tec->simulate) {
...
_tenEstimateNumUpdate(tenEstimateContext *tec) {
  ...
  if (tec->flag[flagBInfo] || tec->flag[flagSkipSet]) {
    if (tec->_ngrad) {
    ...
    airFree(tec->skipLut);
    tec->skipLut = AIR_CAST(unsigned char *, calloc(tec->allNum, sizeof(unsigned char)));
    airFree(tec->bnorm);
    ...
    for (skipListIdx = 0; skipListIdx < tec->skipListArr->len; skipListIdx++) {
      ...
      if (!(skipIdx < tec->allNum)) {
        biffAddf(TEN, "%s: skipList entry %u value index %u not < # vals %u", me,
                 skipListIdx, skipIdx, tec->allNum);
        return 1;
      }
    ...
    if (!(skipNotNum >= 7)) {
      biffAddf(TEN, "%s: number of not-skipped (used) values %u < minimum 7", me,
               skipNotNum);
      return 1;
    }
    ...
    for (allIdx = 0; allIdx < tec->allNum; allIdx++) {
      if (tec->skipLut[allIdx]) {
        ...
        tec->bnorm[allIdx] = sqrt(bb[0] * bb[0] + 2 * bb[1] * bb[1] + 2 * bb[2] * bb[2]
                                  + bb[3] * bb[3] + 2 * bb[4] * bb[4] + bb[5] * bb[5]);
        if (tec->estimateB0) {
          ++newDwiNum;
        } else {
...
_tenEstimateAllAllocUpdate(tenEstimateContext *tec) {
  ...
  if (tec->flag[flagAllNum]) {
    ...
    if (!(tec->all && tec->allTmp)) {
      biffAddf(TEN, "%s: couldn't allocate \"all\" arrays (length %u)", me, tec->allNum);
      return 1;
    }
...
_tenEstimateDwiAllocUpdate(tenEstimateContext *tec) {
  ...
  if (tec->flag[flagDwiNum]) {
    ...
    if (!(tec->dwi && tec->dwiTmp)) {
      biffAddf(TEN, "%s: couldn't allocate DWI arrays (length %u)", me, tec->dwiNum);
      return 1;
    }
    E = 0;
...
_tenEstimateWghtUpdate(tenEstimateContext *tec) {
  ...
  if (tec->flag[flagDwiAlloc] || tec->flag[flagEstimateMethod]) {
...
_tenEstimateValuesSet(tenEstimateContext *tec) {
  ...
  for (allIdx = 0; allIdx < tec->allNum; allIdx++) {
    if (!tec->skipLut[allIdx]) {
      tec->all[allIdx] = (tec->all_f ? tec->all_f[allIdx] : tec->all_d[allIdx]);
  ...
  if (tec->dwiConfSoft > 0) {
    tec->conf = AIR_AFFINE(-1,
                           airErf((tec->mdwi - tec->dwiConfThresh) / tec->dwiConfSoft),
                           1, 0, 1);
  } else {
...
_tenEstimate1TensorSimulateSingle(tenEstimateContext *tec, double sigma, double bValue,
  ...
  if (!(AIR_EXISTS(sigma) && sigma >= 0 && AIR_EXISTS(bValue) && AIR_EXISTS(B0))) {
...
tenEstimate1TensorSimulateVolume(tenEstimateContext *tec, Nrrd *ndwi, double sigma,
  ...
  if (!(AIR_EXISTS(sigma) && sigma >= 0.0 && AIR_EXISTS(bValue) && bValue >= 0.0)) {
  ...
  if (!(nrrdTypeFloat == outType || nrrdTypeDouble == outType)) {
    biffAddf(TEN, "%s: requested output type (%s) not %s or %s", me,
             airEnumStr(nrrdType, outType), airEnumStr(nrrdType, nrrdTypeFloat),
  ...
  if (!(3 == nB0->dim && sizeX == nB0->axis[0].size && sizeY == nB0->axis[1].size
  ...
  for (II = 0; !E && II < NN; II++) {
    ...
    if (nrrdTypeDouble == outType) {
      ...
      E = tenEstimate1TensorSimulateSingle_d(tec, dwi_d, sigma, bValue, B0, ten_d);
      dwi_d += tec->allNum;
    } else {
      for (tt = 0; tt < 7; tt++) {
        ten_f[tt] = AIR_FLOAT(tlup(nten->data, tt + sizeTen * II));
      }
      E = tenEstimate1TensorSimulateSingle_f(tec, dwi_f, AIR_FLOAT(sigma),
                                             AIR_FLOAT(bValue), AIR_FLOAT(B0), ten_f);
      dwi_f += tec->allNum;
    }
    if (E) {
      biffAddf(TEN, "%s: failed at sample %s", me, airSprintSize_t(stmp[0], II));
      airMopError(mop);
      return 1;
    }
  }
  ...
  if (nrrdBasicInfoCopy(ndwi, nten, NRRD_BASIC_INFO_ALL ^ NRRD_BASIC_INFO_SPACE)) {
  ...
  if (keyValueSet) {
    ...
    if (tec->_ngrad) {
      ...
      for (allIdx = 0; allIdx < tec->allNum; allIdx++) {
        ...
        sprintf(valstr, "%g %g %g", lup(tec->_ngrad->data, 0 + 3 * allIdx),
      ...
      for (allIdx = 0; allIdx < tec->allNum; allIdx++) {
        ...
        sprintf(valstr, "%g %g %g %g %g %g", lup(tec->_nbmat->data, 0 + 6 * allIdx),
  ...
  if (tec->estimateB0) {
    ...
    for (jj = 0; jj < 7; jj++) {
      ...
      if (jj < 6) {
        tec->ten[1 + jj] = tmp;
        if (!AIR_EXISTS(tmp)) {
          biffAddf(TEN, "%s: estimated non-existent tensor coef (%u) %g", me, jj, tmp);
          return 1;
        }
      } else {
        ...
        if (!AIR_EXISTS(tec->estimatedB0)) {
          biffAddf(TEN, "%s: estimated non-existent B0 %g (b=%g, tmp=%g)", me,
                   tec->estimatedB0, tec->bValue, tmp);
          return 1;
        }
      }
    }
      ...
      for (ii = 0; ii < tec->dwiNum; ii++) {
        ...
        if (tec->verbose > 5) {
          fprintf(stderr, "%s: emat[(%u,%u)=%u]*dwi[%u] = %g*%g --> %g\n", me, ii, jj,
                  ii + tec->dwiNum * jj, ii, emat[ii + tec->dwiNum * jj],
                  tec->dwiTmp[ii], tmp);
        }
      }
      tec->ten[1 + jj] = tmp;
...
_tenEstimate1Tensor_WLS(tenEstimateContext *tec) {
  ...
  if (ell_Nm_wght_pseudo_inv(tec->nemat, tec->nbmat, tec->nwght)) {
    biffMovef(TEN, ELL, "%s(1): trouble wght-pseudo-inverting %ux%u B-matrix", me,
              AIR_UINT(tec->nbmat->axis[1].size), AIR_UINT(tec->nbmat->axis[0].size));
    return 1;
  }
  /*
  ...
  for (iter = 0; iter < tec->WLSIterNum; iter++) {
    ...
    */
    if (_tenEstimate1TensorSimulateSingle(tec, 0.0, tec->bValue,
                                          (tec->estimateB0 ? tec->estimatedB0
                                                           : tec->knownB0),
                                          tec->ten)) {
    ...
    for (dwiIdx = 0; dwiIdx < tec->dwiNum; dwiIdx++) {
      ...
      if (!AIR_EXISTS(dwi)) {
        biffAddf(TEN, "%s: bad simulated dwi[%u] == %g (iter %u)", me, dwiIdx, dwi,
                 iter);
        return 1;
      }
...
_tenEstimate1TensorDescent(tenEstimateContext *tec,
  ...
  double currB0, lastB0, currTen[7], lastTen[7], gradB0 = AIR_NAN, gradTen[7], epsilon,
                                                 stepSize, badInit, bad, badDelta,
                                                 stepSizeMin = 0.00000000001, badLast;
  ...
  if (tec->verbose) {
    fprintf(stderr, "%s: WLS gave %g %g %g    %g %g    %g\n", me, tec->ten[1],
            tec->ten[2], tec->ten[3], tec->ten[4], tec->ten[5], tec->ten[6]);
  }

  if (badnessCB(tec, &badInit, (tec->estimateB0 ? tec->estimatedB0 : tec->knownB0),
                tec->ten)
      || !AIR_EXISTS(badInit)) {
...
newepsilon:
  if (_tenEstimate1TensorGradient(tec, &gradB0, gradTen,
                                  ...
                                  tec->ten, epsilon, gradientCB, badnessCB)) {
    biffAddf(TEN, "%s: problem getting initial gradient", me);
    return 1;
  }
  if (!(AIR_EXISTS(gradB0) || 0 <= TEN_T_NORM(gradTen))) {
    biffAddf(TEN, "%s: got bad gradB0 %g or zero-norm tensor grad", me, gradB0);
    return 1;
  }
  if (tec->verbose) {
    fprintf(stderr, "%s: gradTen (%s) = %g %g %g  %g %g   %g\n", me,
            gradientCB ? "analytic" : "cent-diff", gradTen[1], gradTen[2], gradTen[3],
  ...
  do {
    ...
    if (badnessCB(tec, &bad, currB0, currTen) || !AIR_EXISTS(bad)) {
  ...
  do {
    ...
    lastB0 = currB0;
    if (0 == (iter % 3)) {
      if (_tenEstimate1TensorGradient(tec, &gradB0, gradTen, currB0, currTen,
                                      stepSize / 5, gradientCB, badnessCB)
          || !AIR_EXISTS(gradB0)) {
  ...
  *retP = _tenEstimateErrorDwi(tec);
  if (tec->verbose > 2) {
    fprintf(stderr, "!%s: badness(%g, (%g) %g %g %g   %g %g  %g) = %g\n", me, currB0,
            currTen[0], currTen[1], currTen[2], currTen[3], currTen[4], currTen[5],
            currTen[6], *retP);
  }
  return 0;
}
...
_tenEstimate1Tensor_GradientMLE(tenEstimateContext *tec, double *gradB0P,
  ...
  if (tec->verbose) {
    fprintf(stderr, "%s grad (currTen = %g %g %g   %g %g    %g)\n", me, currTen[1],
            currTen[2], currTen[3], currTen[4], currTen[5], currTen[6]);
  }
  TEN_T_SET(gradTen, 0, 0, 0, 0, 0, 0, 0);
  *gradB0P = 0;
  sigma = tec->sigma;
  ...
  for (dwiIdx = 0; dwiIdx < tec->dwiNum; dwiIdx++) {
    ...
    if (tec->verbose) {
      fprintf(stderr, "%s[%u]: bmat = %g %g %g    %g %g     %g\n", me, dwiIdx, bmat[0],
              bmat[1], bmat[2], bmat[3], bmat[4], bmat[5]);
      fprintf(stderr, "%s[%u]: scl = %g -> gradTen = %g %g %g    %g %g   %g\n", me,
              dwiIdx, scl, gradTen[1], gradTen[2], gradTen[3], gradTen[4], gradTen[5],
              gradTen[6]);
    }
    if (!AIR_EXISTS(scl)) {
      TEN_T_SET(gradTen, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN);
      *gradB0P = AIR_NAN;
  ...
  if (tec->verbose) {
    fprintf(stderr, "%s: final gradTen = %g %g %g    %g %g   %g\n", me, gradTen[1],
            gradTen[2], gradTen[3], gradTen[4], gradTen[5], gradTen[6]);
  }
  return 0;
}
#endif
...
_tenEstimate1Tensor_BadnessMLE(tenEstimateContext *tec, double *retP, double currB0,
  ...
  if (E) {
    biffAddf(TEN, "%s[%u]: dot = (%g %g %g %g %g %g).(%g %g %g %g %g %g) = %g", me,
             dwiIdx, bmat[0], bmat[1], bmat[2], bmat[3], bmat[4], bmat[5], curt[1],
             curt[2], curt[3], curt[4], curt[5], curt[6], dot);
    biffAddf(TEN,
             "%s[%u]: simdwi = %g * exp(-%g * %g) = %g * exp(%g) "
             "= %g * %g = %g",
             me, dwiIdx, currB0, tec->bValue, dot, currB0, -(tec->bValue) * dot, currB0,
             exp(-(tec->bValue) * dot), currB0 * exp(-(tec->bValue) * dot));
    biffAddf(TEN, "%s[%u]: mesdwi = %g, simdwi = %g, sigma = %g", me, dwiIdx, mesdwi,
             simdwi, tec->sigma);
    biffAddf(TEN, "%s[%u]: rice = %g, logrice = %g, sum = %g", me, dwiIdx, rice, logrice,
             sum);
    *retP = AIR_NAN;
    return 1;
  }
...
_tenEstimate1TensorSingle(tenEstimateContext *tec) {
  ...
  case tenEstimate1MethodMLE:
    ...
    break;
  default:
    biffAddf(TEN, "%s: estimation method %d unimplemented", me, tec->estimate1Method);
    return 1;
  }
  ...
  if (tec->recordErrorDwi || tec->recordErrorLogDwi) {
    B0 = tec->estimateB0 ? tec->estimatedB0 : tec->knownB0;
    if (_tenEstimate1TensorSimulateSingle(tec, 0.0, tec->bValue, B0, tec->ten)) {
...
tenEstimate1TensorSingle_d(tenEstimateContext *tec, double ten[7], const double *all) {
  ...
  if (tec->verbose) {
    ...
    fprintf(stderr,
            "%s: will estimate by %d (%s) \n"
            "  estimateB0 %d; valueMin %g\n",
            me, tec->estimate1Method,
            airEnumStr(tenEstimate1Method, tec->estimate1Method), tec->estimateB0,
            tec->valueMin);
  }
  if (_tenEstimate1TensorSingle(tec)) {
    biffAddf(TEN, "%s: ", me);
  ...
  if (tec->verbose) {
    fprintf(stderr, "%s: ten = %g   %g %g %g   %g %g   %g\n", me, tec->ten[0],
            tec->ten[1], tec->ten[2], tec->ten[3], tec->ten[4], tec->ten[5],
            tec->ten[6]);
  }
...
tenEstimate1TensorVolume4D(tenEstimateContext *tec, Nrrd *nten, Nrrd **nB0P,
  ...
  double *all, ten[7], (*lup)(const void *, size_t), (*ins)(void *v, size_t I, double d);
  unsigned int dd;
  airArray *mop;
  int axmap[4];
...
#  define NUM 800
  ...
  if (tec->allNum != ndwi->axis[0].size) {
    biffAddf(TEN,
             "%s: from %s info, expected %u values per sample, "
             "but have %s in volume",
             me, tec->_ngrad ? "gradient" : "B-matrix", tec->allNum,
  ...
  if (nterrP) {
    ...
    if (1 != recE + recEL + recLK) {
      biffAddf(TEN,
               "%s: requested error volume but need exactly one of "
  ...
  if (nrrdMaybeAlloc_va(nten, outType, 4, sizeTen, sizeX, sizeY, sizeZ)) {
  ...
  if (nB0P) {
    *nB0P = nrrdNew();
    if (nrrdMaybeAlloc_va(*nB0P, outType, 3, sizeX, sizeY, sizeZ)) {
  ...
  if (nterrP) {
    *nterrP = nrrdNew();
    if (nrrdMaybeAlloc_va(*nterrP, outType, 3, sizeX, sizeY, sizeZ)
  ...
  for (II = 0; II < NN; II++) {
    ...
    */
    ...
    if (tenEstimate1TensorSingle_d(tec, ten, all)) {
      biffAddf(TEN, "%s: failed at sample %s", me, airSprintSize_t(stmp, II));
      airMopError(mop);
      return 1;
    }
    ...
    if (nB0P) {
      ins((*nB0P)->data, II, (tec->estimateB0 ? tec->estimatedB0 : tec->knownB0));
    }
    if (nterrP) {
