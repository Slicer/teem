...
tenGradientMeasure(double *pot, double *minAngle, double *minEdge, const Nrrd *npos,
  ...
  edge = (edgeNormalize ? tenGradientIdealEdge(num, tgparm->single) : 1.0);
  *pot = 0;
  if (minAngle) {
    *minAngle = AIR_PI;
  }
...
_tenGradientUpdate(double *meanVel, double *edgeMin, Nrrd *npos, double edge,
  ...
  *edgeMin = edge;
  ...
  limit = expo * AIR_MIN(sqrt(expo), log(1 + tgparm->initStep / tgparm->step));
  for (ii = 0; ii < num; ii++) {
    ...
    ELL_3V_SCALE_ADD2(newpos, 1.0, pos + 3 * ii, step, ngrad);
...
tenGradientBalance(Nrrd *nout, const Nrrd *nin, tenGradientParm *tgparm) {
  ...
  if (!nout || tenGradientCheck(nin, nrrdTypeUnknown, 2) || !tgparm) {
    biffAddf(TEN, "%s: got NULL pointer (%p,%p) or invalid nin", me, AIR_VOIDP(nout),
             AIR_VOIDP(tgparm));
    return 1;
  }
  if (nrrdConvert(nout, nin, nrrdTypeDouble)) {
  ...
  do {
    ...
    if (iter >= maxIter) {
      ...
      done = (improv <= tgparm->minMeanImprovement || len < tgparm->minMean);
    }
  } while (!done);
...
tenGradientDistribute(Nrrd *nout, const Nrrd *nin, tenGradientParm *tgparm) {
  ...
  airArray *mop;
  Nrrd *npos[2];
  double *pos, len, meanVelocity, pot, potNew, potD, edge, edgeMin, angle, angleNew;
  int E;
  ...
  tgparm->nudge = 0.1;
  tenGradientMeasure(&pot, &angle, NULL, npos[oldIdx], tgparm, AIR_TRUE);
  for (iter = 0; ((!!tgparm->minIteration && iter < tgparm->minIteration)
                  || (iter < tgparm->maxIteration
                      && (!tgparm->minPotentialChange || !AIR_EXISTS(potD)
    ...
    do {
      E = _tenGradientUpdate(&meanVelocity, &edgeMin, npos[newIdx], edge, tgparm);
      if (E) {
        if (edgeShrink > tgparm->maxEdgeShrink) {
          biffAddf(TEN, "%s: %u > %u edge shrinks (%g), update still failed", me,
                   edgeShrink, tgparm->maxEdgeShrink, edge);
          airMopError(mop);
          return 1;
        }
        edgeShrink++;
    ...
    tenGradientMeasure(&potNew, &angleNew, NULL, npos[newIdx], tgparm, AIR_TRUE);
    if ((AIR_EXISTS(pot) && AIR_EXISTS(potNew) && potNew <= pot) || angleNew >= angle) {
      ...
      if (!(iter % tgparm->report) && tgparm->verbose) {
        fprintf(stderr,
                "%s(%d): . . . . . . step = %g, edgeShrink = %u\n"
                "   velo = %g<>%g, phi = %g ~ %g<>%g, angle = %g ~ %g\n",
                me, iter, tgparm->step, edgeShrink, meanVelocity, tgparm->minVelocity,
                pot, potD, tgparm->minPotentialChange, angle, angleNew - angle);
      }
      if (tgparm->snap && !(iter % tgparm->snap)) {
        ...
        if (nrrdSave(filename, npos[newIdx], NULL)) {
          ...
          if (tgparm->verbose) { /* perhaps shouldn't have this check */
            fprintf(stderr,
                    "%s: iter=%d, couldn't save snapshot:\n%s"
                    "continuing ...\n",
                    me, iter, serr);
          }
          free(serr);
      ...
      if (tgparm->verbose) {
        fprintf(stderr,
                "%s(%d): ######## step %g --> %g\n"
                " phi = %g --> %g ~ %g, angle = %g --> %g\n",
                me, iter, tgparm->step, tgparm->step / 2, pot, potNew, potD, angle,
                angleNew);
      }
      tgparm->step /= 2;
      tgparm->nudge /= 2;
  ...
  if (tgparm->verbose) {
    fprintf(stderr,
            ...
            "               && (%d || %d) \n"
            "               && %d) is false\n",
            me, !!tgparm->minIteration,
            iter<tgparm->minIteration,
                 iter<tgparm->maxIteration, !tgparm->minPotentialChange,
                      !AIR_EXISTS(potD), AIR_ABS(potD)>
                   tgparm->minPotentialChange,
                 !tgparm->minVelocity, meanVelocity>
              tgparm->minVelocity,
            tgparm->step > FLT_MIN);
    fprintf(stderr, "  iter=%d, velo = %g<>%g, phi = %g ~ %g<>%g;\n", iter, meanVelocity,
            tgparm->minVelocity, pot, potD, tgparm->minPotentialChange);
  ...
  if ((tgparm->minMeanImprovement || tgparm->minMean) && !tgparm->single) {
    if (tgparm->verbose) {
...
tenGradientGenerate(Nrrd *nout, unsigned int num, tenGradientParm *tgparm) {
  ...
  if (!(num >= 3)) {
    biffAddf(TEN,
             "%s: can generate minimum of 3 gradient directions "
             "(not %d)",
             me, num);
    return 1;
  }
  ...
  if (tgparm->insertZeroVec) {
    ...
    if (nrrdPad_nva(ntmp, nout, padMin, padMax, nrrdBoundaryPad, 0.0)
        || nrrdCopy(nout, ntmp)) {
