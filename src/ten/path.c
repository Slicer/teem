...
tenInterpParmBufferAlloc(tenInterpParm *tip, unsigned int num) {
  ...
  if (0 == num) {
    ...
    if (!(tip->eval && tip->evec && tip->rtIn && tip->rtLog && tip->qIn && tip->qBuff
          && tip->qInter)) {
      biffAddf(TEN, "%s: didn't alloc buffers (%p,%p,%p %p %p %p %p)", me,
               AIR_VOIDP(tip->eval), AIR_VOIDP(tip->evec), AIR_VOIDP(tip->rtIn),
               AIR_VOIDP(tip->rtLog), AIR_VOIDP(tip->qIn), AIR_VOIDP(tip->qBuff),
...
*/
void
tenInterpTwo_d(double oten[7], const double tenA[7], const double tenB[7], int ptype,
               double aa, tenInterpParm *tip) {
  static const char me[] = "tenInterpTwo_d";
  double logA[7], logB[7], tmp1[7], tmp2[7], logMean[7], mat1[9], mat2[9], mat3[9],
    sqrtA[7], isqrtA[7], mean[7], sqrtB[7], isqrtB[7], oeval[3], evalA[3], evalB[3],
    oevec[9], evecA[9], evecB[9];

  if (!(oten && tenA && tenB)) {
    /* got NULL pointer, but not using biff */
    if (oten) {
      TEN_T_SET(oten, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN);
    }
    return;
  }
  ...
  case tenInterpTypeAffineInvariant:
    ...
    if (tip->verbose) {
      fprintf(stderr,
              ...
              "foo = %g %g %g   %g %g  %g\n"
              "bar(%g) = %g %g %g   %g %g  %g\n",
              me, tenA[1], tenA[2], tenA[3], tenA[4], tenA[5], tenA[6], tenB[1], tenB[2],
              tenB[3], tenB[4], tenB[5], tenB[6], tmp1[1], tmp1[2], tmp1[3], tmp1[4],
              tmp1[5], tmp1[6], aa, oten[1], oten[2], oten[3], oten[4], oten[5],
              oten[6]);
    }
    break;
  ...
  case tenInterpTypeRThetaPhiLinear:
    if (1) {
      ...
      tenTripleConvertSingle_d(rtpA, tenTripleTypeRThetaPhi, eval,
                               tenTripleTypeEigenvalue);
      tenEigensolve_d(eval, NULL, tenB);
      tenTripleConvertSingle_d(rtpB, tenTripleTypeRThetaPhi, eval,
                               tenTripleTypeEigenvalue);
      TEN_T_LERP(tenM, aa, tenA, tenB);
      tenEigensolve_d(eval, oevec, tenM);
      ELL_3V_LERP(rtpM, aa, rtpA, rtpB);
      tenTripleConvertSingle_d(oeval, tenTripleTypeEigenvalue, rtpM,
...
tenInterpN_d(double tenOut[7], const double *tenIn, const double *wght, unsigned int num,
  ...
  TEN_T_SET(tenErr, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN);
  ...
  if (!(AIR_IN_CL(1 - tip->wghtSumEps, wghtSum, 1 + tip->wghtSumEps))) {
    biffAddf(TEN, "%s: wght sum %g not within %g of 1.0", me, wghtSum, tip->wghtSumEps);
    TEN_T_COPY(tenOut, tenErr);
    return 1;
  }
  ...
  case tenInterpTypeRThetaPhiLinear:
    ...
    for (ii = 0; ii < num; ii++) {
      ...
      tenTripleConvertSingle_d(tmprtp, tenTripleTypeRThetaPhi, tmpeval,
                               tenTripleTypeEigenvalue);
      ww = wght ? wght[ii] : 1.0 / num;
    ...
    tenTripleConvertSingle_d(eval, tenTripleTypeEigenvalue, rtp, tenTripleTypeRThetaPhi);
    tenMakeSingle_d(tenOut, tmpTen[0], eval, evec);
    break;
  default:
    biffAddf(TEN, "%s: sorry, interp type %s (%d) not implemented", me,
             airEnumStr(tenInterpType, ptype), ptype);
    TEN_T_COPY(tenOut, tenErr);
    return 1;
  }
...
_tenInterpGeoLoxRelaxOne(Nrrd *nodata, Nrrd *ntdata, Nrrd *nigrtdata, unsigned int ii,
  ...
  double *tdata, *odata, *igrtdata, *tt[5], *igrt[5][6], d02[7], d24[7], len02, len24,
    tmp, tng[7], correct, update[7];
  unsigned int jj;

  if (tip->verbose) {
    fprintf(stderr, "---- %u --> %u %u %u %u %u\n", ii, 2 * ii - 2, 2 * ii - 1, 2 * ii,
            2 * ii + 1, 2 * ii + 2);
  }
  ...
  for (jj = 0; jj < (rotnoop ? 3u : 6u); jj++) {
    ...
    if (tip->verbose) {
      fprintf(stderr, "igrt[1][%u] = %g %g %g   %g %g   %g\n", jj, igrt[1][jj][1],
              igrt[1][jj][2], igrt[1][jj][3], igrt[1][jj][4], igrt[1][jj][5],
              igrt[1][jj][6]);
      fprintf(stderr, "igrt[3][%u] = %g %g %g   %g %g   %g\n", jj, igrt[3][jj][1],
              igrt[3][jj][2], igrt[3][jj][3], igrt[3][jj][4], igrt[3][jj][5],
              igrt[3][jj][6]);
      fprintf(stderr,
              "(jj=%u) len = %g %g --> (d = %g) "
              "update = %g %g %g     %g %g   %g\n",
              jj, len02, len24, TEN_T_DOT(igrt[2][0], update), update[1], update[2],
              update[3], update[4], update[5], update[6]);
    }
  }
  if (rotnoop) {
    ...
    if (tip->verbose) {
      fprintf(stderr,
              "(rotnoop) (d = %g) "
              "update = %g %g %g     %g %g   %g\n",
              TEN_T_DOT(igrt[2][0], update), update[1], update[2], update[3], update[4],
              update[5], update[6]);
    }
  }
  /*
...
_tenInterpGeoLoxIGRT(double *igrt, double *ten, int useK, int rotNoop, double minnorm) {
  ...
  if (rotNoop) {
    ...
    TEN_T_SET(igrt + 7 * 3, 1, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN);
    TEN_T_SET(igrt + 7 * 4, 1, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN);
    TEN_T_SET(igrt + 7 * 5, 1, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN, AIR_NAN);
  } else {
...
_tenPathSpacingEqualize(Nrrd *nout, Nrrd *nin) {
  ...
  */
    ...
    if (lenRmn + lenHere >= lenStep) {
      ...
      while (len > lenStep) {
        ...
        fprintf(stderr, "!%s(%u): len = %g -> %g\n", me, idxIn,
                len + lenStep, len);
        */
        TEN_T_AFFINE(out + 7 * (2 * idxOut + 0), lenHere, len, 0, tenHere, tenNext);
        /*
  ...
  for (idxOut = 0; idxOut < NN; idxOut++) {
    TEN_T_LERP(out + 7 * (2 * idxOut + 1), 0.5, out + 7 * (2 * idxOut + 0),
               out + 7 * (2 * idxOut + 2));
  }
  return lenTotal;
}
...
_tenInterpGeoLoxPolyLine(Nrrd *ngeod, unsigned int *numIter, const double tenA[7],
  ...
  *numIter = 0;
  ...
  for (ii = 0; ii <= 2 * NN; ii++) {
    _tenInterpGeoLoxIGRT(igrt + 7 * 6 * ii, tt + 7 * ii, useK, rotnoop, tip->minNorm);
  }
  nrrdCopy(nss, ntt);
  ...
  do {
    ...
    for (ii = lo; ii != hi; ii += dd) {
      ...
      if (_tenInterpGeoLoxRelaxOne(nss, ntt, nigrt, ii, rotnoop, sclHack * tip->convStep,
                                   tip)) {
...
tenInterpTwoDiscrete_d(Nrrd *nout, const double tenA[7], const double tenB[7], int ptype,
  ...
  if (airEnumValCheck(tenInterpType, ptype)) {
    biffAddf(TEN, "%s: path type %d not a valid %s", me, ptype, tenInterpType->name);
    return 1;
  }
  ...
  if (ptype == tenInterpTypeLinear || ptype == tenInterpTypeLogLinear
      || ptype == tenInterpTypeAffineInvariant || ptype == tenInterpTypeWang
      || ptype == tenInterpTypeQuatGeoLoxK || ptype == tenInterpTypeQuatGeoLoxR
    ...
    for (ii = 0; ii < num; ii++) {
      ...
      tenInterpTwo_d(out + 7 * ii, tenA, tenB, ptype, (double)ii / (num - 1), tip);
    }
  } else if (ptype == tenInterpTypeGeoLoxK || ptype == tenInterpTypeGeoLoxR
             || ptype == tenInterpTypeLoxK || ptype == tenInterpTypeLoxR) {
    /* we have slow iterative code for these */
    unsigned int numIter;
    int useK, rotnoop;

    useK = (tenInterpTypeGeoLoxK == ptype || tenInterpTypeLoxK == ptype);
    rotnoop = (tenInterpTypeGeoLoxK == ptype || tenInterpTypeGeoLoxR == ptype);
    fprintf(stderr, "!%s: useK = %d, rotnoop = %d\n", me, useK, rotnoop);
    if (_tenInterpGeoLoxPolyLine(nout, &numIter, tenA, tenB, num, useK, rotnoop, tip)) {
...
tenInterpDistanceTwo_d(const double tenA[7], const double tenB[7], int ptype,
  ...
  tenInterpParm *tip;
  airArray *mop;
  double ret, diff[7], logA[7], logB[7], invA[7], det, siA[7], mat1[9], mat2[9], mat3[9],
    logDiff[7];
  Nrrd *npath;
  ...
  case tenInterpTypeQuatGeoLoxR:
    ...
    if (tenInterpTwoDiscrete_d(npath, tenA, tenB, ptype, tip->numSteps, tip)) {
    ...
    if (tip->lengthFancy) {
      tip->lengthShape = tenInterpPathLength(npath, AIR_FALSE, AIR_TRUE, AIR_TRUE);
      tip->lengthOrient = tenInterpPathLength(npath, AIR_FALSE, AIR_TRUE, AIR_FALSE);
    }
    break;
  case tenInterpTypeWang:
  default:
    fprintf(stderr, "%s: unimplemented %s %d!!!!\n", me, tenInterpType->name, ptype);
    ret = AIR_NAN;
    break;
  }
...
tenInterpMulti3D(Nrrd *nout, const Nrrd *const *nin, const double *wght,
  ...
  unsigned int ninIdx;
  size_t II, NN;
  double (*lup)(const void *, size_t), (*ins)(void *, size_t, double), *tbuff;
  tenInterpParm *tip;
  airArray *mop;
  ...
  if (!(nrrdTypeFloat == nin[0]->type || nrrdTypeDouble == nin[0]->type)) {
  ...
  for (ninIdx = 1; ninIdx < ninLen; ninIdx++) {
    ...
    if (nin[0]->type != nin[ninIdx]->type) {
      biffAddf(TEN, "%s: nin[0] type (%s) != nin[%u] type (%s)", me,
               airEnumStr(nrrdType, nin[0]->type), ninIdx,
               airEnumStr(nrrdType, nin[ninIdx]->type));
      return 1;
    }
  }
