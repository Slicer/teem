...
void qball( const double b, const int gradcount, const double svals[], const double grads[], double qvals[] )
{
	/// Not an optimal Q-ball implementation! (Taken from submission to MICCAI 2006)
	/// Should be solved analytically in the future, implemented from recent papers.
	int i,j;
...
void segsamp2( const int gradcount, const double qvals[], const double grads[], const double qpoints[], unsigned int seg[], double dists[] )
{
...
void initcent2( const int gradcount, const double qvals[], const double qpoints[], double centroids[6] )
{
	int i, maxidx;
	double max, dist;
	...
	//	printf( "init: max=%d cent0=[%f %f %f]\n", maxidx, centroids[0], centroids[1], centroids[2]);
...
/// Calculates 2 new centroids (and a new segmentation) from distances between Q-balls and centroids, returns true if segmentation changed
int calccent2( const int gradcount, const double qpoints[], const double dists[], double centroid[6], unsigned int seg[] )
{
/*
  // Attempt to implement better line-adding by adding outerproducts of points and estimating major eigenvector afterwards
	...
	//ELL_3V_SCALE( centroid, ELL_3V_LEN( centroid ), centroid ); //Normalize
...
void qvals2points( const int gradcount, const double qvals[], const double grads[], double qpoints[] )
{
	int i;
...
/// Calculates the shortest distances from each centroid/axis to each Q-ball point
void calcdists( const int centcount, const double centroid[], const int gradcount, const double qpoints[], double dists[] )
{
	int i,j;
...
/// Estimates the shortest distance from a point to a line going through the origin
double pldist( const double point[], const double line[] )
{
...
void seg2weights( const int gradcount, const int seg[], const int segcount, double weights[] )
{
	int i,j;
