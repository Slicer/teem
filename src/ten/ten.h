...
typedef struct {
  ...
  double fiberTen[7], fiberEval[3], fiberEvec[9], fiberAnisoStop, fiberAnisoSpeed;
...
typedef struct {
  ...
  int simulate,           /* if non-zero, we're being used for simulation,
    ...
    recordErrorDwi, recordErrorLogDwi, recordLikelihoodDwi, verbose, /* blah blah blah */
...
typedef struct {
  ...
  double levmarTau, levmarEps1, levmarEps2, levmarEps3, levmarDelta, levmarMinCp;
...
#define TEN_MODEL_B0_MAX                                                                \
  65500 /* HEY: fairly arbitrary, but is set to be                                      \
           a little below the max storable value                                        \
           of the unsigned short in which DWI                                           \
           values might be saved */
#define TEN_MODEL_DIFF_MAX                                                              \
  0.006                                  /* in units of mm^2/sec; diffusivity of        \
...
typedef struct tenModel_t {
  ...
  double (*sqe)(const double *parm, const tenExperSpec *espec, double *dwiBuff,
                const double *dwiMeas, int knownB0);
  void (*sqeGrad)(double *grad, const double *parm, const tenExperSpec *espec,
  ...
  double (*sqeFit)(double *parm, double *convFrac, unsigned int *itersTaken,
                   ...
                   const double *parmInit, int knownB0, unsigned int minIter,
                   unsigned int maxIter, double convEps, int verbose);
  /* "nll" == negative log likelihood */
  double (*nll)(const double *parm, const tenExperSpec *espec, double *dwiBuff,
                const double *dwiMeas, int rician, double sigma, int knownB0);
  void (*nllGrad)(double *grad, const double *parm, const tenExperSpec *espec,
                  double *dwiBuff, const double *dwiMeas, int rician, double sigma);
  double (*nllFit)(double *parm, const tenExperSpec *espec, const double *dwiMeas,
                   const double *parmInit, int rician, double sigma, int knownB0);
} tenModel;

/* defaultsTen.c */
TEN_EXPORT const int tenPresent;
TEN_EXPORT const char *const tenBiffKey;
...
TEN_EXPORT void tenGradientMeasure(double *pot, double *minAngle, double *minEdge,
...
TEN_EXPORT void tenInterpTwo_d(double oten[7], const double tenA[7],
                               const double tenB[7], int ptype, double aa,
                               tenInterpParm *tip);
TEN_EXPORT int tenInterpN_d(double tenOut[7], const double *tenIn, const double *wght,
                            unsigned int num, int ptype, tenInterpParm *tip);
TEN_EXPORT double tenInterpPathLength(Nrrd *npath, int doubleVerts, int fancy,
                                      int shape);
TEN_EXPORT int tenInterpTwoDiscrete_d(Nrrd *nout, const double tenA[7],
                                      const double tenB[7], int ptype, unsigned int num,
                                      tenInterpParm *tip);
TEN_EXPORT double tenInterpDistanceTwo_d(const double tenA[7], const double tenB[7],
                                         int ptype, tenInterpParm *tip);
TEN_EXPORT int tenInterpMulti3D(Nrrd *nout, const Nrrd *const *nin, const double *wght,
                                unsigned int ninNum, int ptype, tenInterpParm *tip);

/* glyph.c */
TEN_EXPORT tenGlyphParm *tenGlyphParmNew(void);
TEN_EXPORT tenGlyphParm *tenGlyphParmNix(tenGlyphParm *parm);
TEN_EXPORT int tenGlyphParmCheck(tenGlyphParm *parm, const Nrrd *nten, const Nrrd *npos,
                                 const Nrrd *nslc);
TEN_EXPORT int tenGlyphGen(limnObject *glyphs, echoScene *scene, tenGlyphParm *parm,
...
TEN_EXPORT void tenGlyphBqdAbcUv(double abc[3], const double uv[2], double betaMax);

/* tensor.c */
TEN_EXPORT int tenVerbose;
TEN_EXPORT void tenRotateSingle_f(float tenOut[7], const float rot[9],
                                  const float tenIn[7]);
...
TEN_EXPORT int tenExpand2D(Nrrd *nout, const Nrrd *nin, double scale, double thresh);
...
TEN_EXPORT void tenMakeSingle_f(float ten[7], float conf, const float eval[3],
                                const float evec[9]);
TEN_EXPORT void tenMakeSingle_d(double ten[7], double conf, const double eval[3],
                                const double evec[9]);
TEN_EXPORT int tenMake(Nrrd *nout, const Nrrd *nconf, const Nrrd *neval,
                       const Nrrd *nevec);
TEN_EXPORT int tenSlice(Nrrd *nout, const Nrrd *nten, unsigned int axis, size_t pos,
                        unsigned int dim);
...
TEN_EXPORT void tenEstimateLinearSingle_f(float *ten, float *B0P, const float *dwi,
                                          const double *emat, double *vbuf,
                                          unsigned int DD, int knownB0, float thresh,
                                          float soft, float b);
TEN_EXPORT void tenEstimateLinearSingle_d(double *ten, double *B0P, const double *dwi,
                                          const double *emat, double *vbuf,
                                          unsigned int DD, int knownB0, double thresh,
...
TEN_EXPORT int tenEstimateLinear3D(Nrrd *nten, Nrrd **nterrP, Nrrd **nB0P,
                                   ...
                                   const Nrrd *nbmat, int knownB0, double thresh,
                                   double soft, double b);
...
TEN_EXPORT int tenEstimateGradientsSet(tenEstimateContext *tec, const Nrrd *ngrad,
                                       double bValue, int estimateB0);
TEN_EXPORT int tenEstimateBMatricesSet(tenEstimateContext *tec, const Nrrd *nbmat,
                                       double bValue, int estimateB0);
...
TEN_EXPORT int tenEstimateThresholdFind(double *threshP, unsigned char *isB0,
                                        Nrrd *nin4d);
TEN_EXPORT int tenEstimateThresholdSet(tenEstimateContext *tec, double thresh,
                                       double soft);
TEN_EXPORT int tenEstimateUpdate(tenEstimateContext *tec);
TEN_EXPORT int tenEstimate1TensorSimulateSingle_f(tenEstimateContext *tec, float *simval,
                                                  float sigma, float bValue, float B0,
                                                  const float _ten[7]);
TEN_EXPORT int tenEstimate1TensorSimulateSingle_d(tenEstimateContext *tec,
                                                  double *simval, double sigma,
                                                  double bValue, double B0,
                                                  const double ten[7]);
TEN_EXPORT int tenEstimate1TensorSimulateVolume(tenEstimateContext *tec, Nrrd *ndwi,
                                                double sigma, double bValue,
                                                const Nrrd *nB0, const Nrrd *nten,
                                                int outType, int keyValueSet);
TEN_EXPORT int tenEstimate1TensorSingle_f(tenEstimateContext *tec, float ten[7],
                                          const float *all);
TEN_EXPORT int tenEstimate1TensorSingle_d(tenEstimateContext *tec, double ten[7],
                                          const double *all);
TEN_EXPORT int tenEstimate1TensorVolume4D(tenEstimateContext *tec, Nrrd *nten,
                                          Nrrd **nB0P, Nrrd **nterrP, const Nrrd *ndwi,
                                          int outType);
...
TEN_EXPORT int tenAnisoHistogram(Nrrd *nout, const Nrrd *nin, const Nrrd *nwght,
                                 int right, int version, unsigned int resolution);
...
TEN_EXPORT void tenEvecRGBSingle_f(float RGB[3], float conf, const float eval[3],
                                   const float evec[3], const tenEvecRGBParm *rgbp);
TEN_EXPORT void tenEvecRGBSingle_d(double RGB[3], double conf, const double eval[3],
...
TEN_EXPORT int tenEvqVolume(Nrrd *nout, const Nrrd *nin, int which, int aniso,
                            int scaleByAniso);
...
TEN_EXPORT int tenFiberStopAnisoSet(tenFiberContext *tfx, int anisoType,
                                    double anisoThresh);
...
TEN_EXPORT int tenFiberAnisoSpeedSet(tenFiberContext *tfx, int aniso, double lerp,
...
/* fiber.c */
TEN_EXPORT int tenFiberTraceSet(tenFiberContext *tfx, Nrrd *nfiber, double *buff,
...
TEN_EXPORT int tenFiberMultiTrace(tenFiberContext *tfx, tenFiberMulti *tfml,
                                  const Nrrd *nseed);
TEN_EXPORT int tenFiberMultiPolyData(tenFiberContext *tfx, limnPolyData *lpld,
                                     tenFiberMulti *tfml);
TEN_EXPORT int tenFiberMultiProbeVals(tenFiberContext *tfx, Nrrd *nval,
                                      tenFiberMulti *tfml);
...
TEN_EXPORT int tenEpiRegister3D(Nrrd **nout, Nrrd **ndwi, unsigned int dwiLen,
                                ...
                                const NrrdKernel *kern, double *kparm, int progress,
                                int verbose);
TEN_EXPORT int tenEpiRegister4D(Nrrd *nout, Nrrd *nin, Nrrd *ngrad, int reference,
                                double bwX, double bwY, double fitFrac, double DWthr,
                                int doCC, const NrrdKernel *kern, double *kparm,
...
TEN_EXPORT int tenModelSimulate(Nrrd *ndwi, int typeOut, tenExperSpec *espec,
                                const tenModel *model, const Nrrd *nB0, /* maybe NULL */
                                const Nrrd *nparm, int keyValueSet);
TEN_EXPORT int tenModelSqeFit(Nrrd *nparm, Nrrd **nsqeP, Nrrd **nconvP, Nrrd **niterP,
                              const tenModel *model, const tenExperSpec *espec,
                              const Nrrd *ndwi, int knownB0, int saveB0, int typeOut,
                              unsigned int minIter, unsigned int maxIter,
                              unsigned int starts, double convEps, airRandMTState *rng,
                              int verbose);
TEN_EXPORT int tenModelNllFit(Nrrd *nparm, Nrrd **nnllP, const tenModel *model,
                              const tenExperSpec *espec, const Nrrd *ndwi, int rician,
                              double sigma, int knownB0);
TEN_EXPORT int tenModelConvert(Nrrd *nparmDst, int *convRet, const tenModel *modelDst,
                               const Nrrd *nparmSrc, const tenModel *modelSrc);
...
TEN_EXPORT int tenAnisoScale(Nrrd *nout, const Nrrd *nin, double scale, int fixDet,
                             int makePositive);
...
/* bvec.c */
TEN_EXPORT int tenBVecNonLinearFit(Nrrd *nout, const Nrrd *nin, double *bb, double *ww,
...
TEN_EXPORT int tenDwiGageKindSet(gageKind *dwiKind, double thresh, double soft,
                                 double bval, double valueMin, const Nrrd *ngrad,
                                 const Nrrd *nbmat, int emethod1, int emethod2,
                                 unsigned int randSeed);
