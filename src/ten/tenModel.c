...
tenModelParse(const tenModel **model, int *plusB0, int requirePrefix, const char *_str) {
  ...
  if (pre) {
    ...
    if (requirePrefix) {
      biffAddf(TEN, "%s: didn't see prefix \"%s\" in \"%s\"", me, tenModelPrefixStr,
               _str);
      airMopError(mop);
      return 1;
    }
...
tenModelFromAxisLearn(const tenModel **modelP, int *plusB0, const NrrdAxisInfo *axinfo) {
  ...
  *plusB0 = AIR_FALSE;
  ...
  if (nrrdKind3DSymMatrix == axinfo->kind || nrrdKind3DMaskedSymMatrix == axinfo->kind) {
...
tenModelSimulate(Nrrd *ndwi, int typeOut, tenExperSpec *espec, const tenModel *model,
  ...
  if (model->parmNum - 1 == gpsze) {
    ...
    biffAddf(TEN,
             "%s: mismatch between getting %u parms, "
             "needing %u (for %s)\n",
             me, gpsze, model->parmNum, model->name);
    return 1;
  } else {
...
_tenModelSqeFitSingle(const tenModel *model, double *testParm, double *grad,
  ...
  do {
    ...
    do {
      ...
      if (verbose > 1) {
        ...
        fprintf(stderr, "%s(%s): (iter %u/%u) tried %s -> %g (step %g)\n", me,
                model->name, iter, subIter, pstr, testval, step);
      }
      if (testval > val) {
        step *= bak;
      }
    ...
    step *= opp;
    iter++;
    done = (iter < minIter ? AIR_FALSE : (iter > maxIter) || dist < convEps);
  } while (!done);
...
tenModelSqeFit(Nrrd *nparm, Nrrd **nsqeP, Nrrd **nconvP, Nrrd **niterP,
               ...
               unsigned int maxIter, unsigned int starts, double convEps,
               airRandMTState *_rng, int verbose) {
  static const char me[] = "tenModelSqeFit";
  char doneStr[13];
  double *ddwi, *dwibuff, sqe, sqeBest, *dparm, *dparmBest,
    (*ins)(void *v, size_t I, double d), (*lup)(const void *v, size_t I);
  airArray *mop;
  ...
  if (espec->imgNum != dwiNum) {
    biffAddf(TEN, "%s: espec expects %u images but dwi has %u on axis 0", me,
             espec->imgNum, AIR_UINT(dwiNum));
    return 1;
  }
  ...
  for (II = 0; II < numSamp; II++) {
    ...
    for (ss = 0; ss < starts; ss++) {
      ...
      sqe = model->sqeFit(dparm, &cvf, &itak, espec, dwibuff, ddwi, dparm, knownB0,
                          minIter, maxIter, convEps, fitVerbose);
      if (sqe <= sqeBest) {
        sqeBest = sqe;
        ...
        || nrrdBasicInfoCopy(nsqe, ndwi,
                             NRRD_BASIC_INFO_DATA_BIT /* */
                               ...
                               | (nrrdStateKeyValuePairsPropagate
                                    ? 0
                                    : NRRD_BASIC_INFO_KEYVALUEPAIRS_BIT))) {
      biffMovef(TEN, NRRD, "%s: couldn't copy axis or basic info to error out", me);
      airMopError(mop);
      return 1;
    }
  }
        ...
        || nrrdBasicInfoCopy(nconv, ndwi,
                             NRRD_BASIC_INFO_DATA_BIT /* */
                               ...
                               | (nrrdStateKeyValuePairsPropagate
                                    ? 0
                                    : NRRD_BASIC_INFO_KEYVALUEPAIRS_BIT))) {
      biffMovef(TEN, NRRD, "%s: couldn't copy axis or basic info to conv out", me);
      airMopError(mop);
      return 1;
    }
  }
        ...
        || nrrdBasicInfoCopy(niter, ndwi,
                             NRRD_BASIC_INFO_DATA_BIT /* */
                               ...
                               | (nrrdStateKeyValuePairsPropagate
                                    ? 0
                                    : NRRD_BASIC_INFO_KEYVALUEPAIRS_BIT))) {
      biffMovef(TEN, NRRD, "%s: couldn't copy axis or basic info to iter out", me);
      airMopError(mop);
      return 1;
    }
  }
  lablen = AIR_UINT((strlen(tenModelPrefixStr) + (saveB0 ? strlen("B0+") : 0)
                     + strlen(model->name) + 1));
  nparm->axis[0].label = AIR_CALLOC(lablen, char);
  sprintf(nparm->axis[0].label, "%s%s%s", tenModelPrefixStr, saveB0 ? "B0+" : "",
          model->name);
...
tenModelConvert(Nrrd *nparmDst, int *convRetP, const tenModel *modelDst,
  ...
  if (!_modelSrc) {
    ...
    if (modelSrc->parmNum - 1 == nparmSrc->axis[0].size) {
      ...
      biffAddf(TEN, "%s: axis[0].size %u is not \"%s\" parmnum %u or 1 less", me,
               AIR_UINT(nparmSrc->axis[0].size), modelSrc->name, modelSrc->parmNum);
      return 1;
    }
  }
  ...
  for (II = 0; II < NN; II++) {
    ...
    if (2 == convRet) { /* HEY should be enum for this value */
      biffAddf(TEN, "%s: error converting from \"%s\" to \"%s\"", me, modelSrc->name,
               modelDst->name);
      airMopError(mop);
      return 1;
    }
  ...
  lablen = AIR_UINT((strlen(tenModelPrefixStr) + (withB0 ? strlen("B0+") : 0)
                     + strlen(modelDst->name) + 1));
  nparmDst->axis[0].label = AIR_CALLOC(lablen, char);
  sprintf(nparmDst->axis[0].label, "%s%s%s", tenModelPrefixStr, withB0 ? "B0+" : "",
          modelDst->name);
