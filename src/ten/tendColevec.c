...
char *_tend_colevecInfoL =
  (INFO
   ". ");

int
tend_colevecMain(int argc, char **argv, char *me, hestParm *hparm) {
  int pret;
  hestOpt *hopt = NULL;
  char *perr, *err;
  airArray *mop;
  ...
  float *cdata, *tdata, eval[3], evec[9], 
    bg, gray, gamma, R, G, B, an[TEN_ANISO_MAX];
  size_t N, I;
  ...
  for (I=0; I<N; I++) {
    ...
    R = AIR_AFFINE(0.0, tdata[0], 1.0, bg, R);
    G = AIR_AFFINE(0.0, tdata[0], 1.0, bg, G);
    B = AIR_AFFINE(0.0, tdata[0], 1.0, bg, B);
