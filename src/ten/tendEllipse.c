...
tend_ellipseDoit(FILE *file, Nrrd *nten, Nrrd *npos, Nrrd *nstn, float min[2],
  ...
  if (npos) {
    ...
    spx = (AIR_EXISTS(nten->axis[1].spacing) ? nten->axis[1].spacing : 1);
    spy = (AIR_EXISTS(nten->axis[2].spacing) ? nten->axis[2].spacing : 1);
    sx = nten->axis[1].size;
    sy = nten->axis[2].size;
    nt = sx * sy;
  ...
  if ((dotRad || lineWidth) && npos && nstn) {
    ...
    for (ti = 0; ti < nstn->axis[1].size; ti++) {
      if (1 == sdata[1 + 3 * ti]) {
        ...
        for (vi = sdata[0 + 3 * ti]; vi < sdata[0 + 3 * ti] + sdata[1 + 3 * ti]; vi++) {
          px = AIR_AFFINE(min[0], pdata[0 + 2 * vi], max[0], minX, maxX);
          py = AIR_AFFINE(min[1], pdata[1 + 2 * vi], max[1], maxY, minY);
          fprintf(file, "%g %g %s\n", px, py,
...
tend_ellipseMain(int argc, const char **argv, const char *me, hestParm *hparm) {
  ...
  if (npos) {
    if (!(2 == nten->dim && 4 == nten->axis[0].size && 2 == npos->dim
          && 2 == npos->axis[0].size && nten->axis[1].size == npos->axis[1].size)) {
  ...
  if (nstn) {
    if (!(nrrdTypeUInt == nstn->type && 2 == nstn->dim && 3 == nstn->axis[0].size)) {
  ...
  tend_ellipseDoit(fout, nten, npos, nstn, min, max, gscale, dotRad, lineWidth, cthresh,
                   invert);
