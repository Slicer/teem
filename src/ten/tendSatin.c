...
tend_satinSphereEigen(float *eval, float *evec, float x, float y, float z, float parm,
  ...
  aniso = AIR_FLOAT(AIR_AFFINE(0.0, AIR_MIN(bound1, bound2), 1.0, mina, maxa));
      ...
      for (xi = 0; xi < size[0]; xi++) {
        ...
        *conf = 1.0;
        if (torus) {
          ...
          tend_satinTorusEigen(eval, evec, x, y, z, parm, mina, maxa, thick,
                               bnd + bndRm * aff, evsc);
        } else {
          float aff;
          aff = AIR_FLOAT(AIR_AFFINE(0, yi, size[1] - 1, 0, 1));
          tend_satinSphereEigen(eval, evec, x, y, z, parm, mina, maxa, thick,
                                bnd + bndRm * aff, evsc);
        }
        conf += 1;
        eval += 3;
...
tend_satinMain(int argc, const char **argv, const char *me, hestParm *hparm) {
  ...
  if (tend_satinGen(nout, parm, mina, maxa, wsize, thick, scaling, bnd, bndRm, evsc,
                    torus)) {
  ...
  ELL_4V_SCALE(spd[1], 32, spd[1]);
  ELL_4MV_COL1_GET(spd[2], shape->ItoW);
  ELL_4V_SCALE(spd[2], 32, spd[2]);
  ELL_4MV_COL2_GET(spd[3], shape->ItoW);
  ELL_4V_SCALE(spd[3], 32, spd[3]);
  ELL_4MV_COL3_GET(orig, shape->ItoW);
  ELL_4V_SCALE(orig, 32, orig);
