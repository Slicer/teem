...
_mu_ev(double mu[3], const double ev[3]) {
  ...
  mu[1] = ((ev[0] - mm) * (ev[0] - mm) + (ev[1] - mm) * (ev[1] - mm)
           + (ev[2] - mm) * (ev[2] - mm))
        / 3;
  mu[2] = ((ev[0] - mm) * (ev[0] - mm) * (ev[0] - mm)
           + (ev[1] - mm) * (ev[1] - mm) * (ev[1] - mm)
           + (ev[2] - mm) * (ev[2] - mm) * (ev[2] - mm))
        / 3;
}

static double _xyzmat[] = {2 / SQRT6,  -1 / SQRT6, -1 / SQRT6, 0,        1 / SQRT2,
                           -1 / SQRT2, 1 / SQRT3,  1 / SQRT3,  1 / SQRT3};
...
#define CONVERT1(dst, mid, src)                                                         \
  static void _##dst##_##src(double dst[3], const double src[3]) {                      \
    double mid[3];                                                                      \
    _##mid##_##src(mid, src);                                                           \
    _##dst##_##mid(dst, mid);                                                           \
  }
...
void
tenTripleConvertSingle_d(double dst[3], int dstType, const double src[3],
                         const int srcType) {
  ...
  if (ELL_3V_EXISTS(src) && !ELL_3V_EXISTS(dst)) {
    fprintf(stderr, "%s: problem? (%s) %g %g %g <-%s- (%s) %g %g %g\n", me,
            airEnumStr(tenTripleType, dstType), dst[0], dst[1], dst[2],
            direct ? "-" : "...", airEnumStr(tenTripleType, srcType), src[0], src[1],
            src[2]);
  }
...
void
tenTripleConvertSingle_f(float _dst[3], int dstType, const float _src[3],
                         const int srcType) {
  double dst[3], src[3];
...
tenTripleCalc(Nrrd *nout, int ttype, const Nrrd *nten) {
  ...
  if (airEnumValCheck(tenTripleType, ttype)) {
    biffAddf(TEN, "%s: got invalid %s (%d)", me, tenTripleType->name, ttype);
    return 1;
  }
  ...
  if (!(nrrdTypeFloat == nten->type || nrrdTypeDouble == nten->type)) {
    biffAddf(TEN, "%s: need input type %s or %s, not %s\n", me,
             airEnumStr(nrrdType, nrrdTypeFloat), airEnumStr(nrrdType, nrrdTypeFloat),
  ...
  if (nrrdBasicInfoCopy(nout, nten, NRRD_BASIC_INFO_ALL ^ NRRD_BASIC_INFO_SPACE)) {
    biffAddf(TEN, "%s:", me);
    return 1;
  }
...
tenTripleConvert(Nrrd *nout, int dstType, const Nrrd *nin, int srcType) {
  ...
  if (airEnumValCheck(tenTripleType, dstType)
      || airEnumValCheck(tenTripleType, srcType)) {
    biffAddf(TEN, "%s: got invalid %s dst (%d) or src (%d)", me, tenTripleType->name,
             dstType, srcType);
    return 1;
  }
  ...
  if (nrrdTypeBlock == nin->type) {
    biffAddf(TEN, "%s: input has non-scalar %s type", me,
